...
| `meshlod.cpp/.hpp` | ~230 | **Bake-time mesh decimation**, shared by the `.tskl` and `.tmdl` bakes (moved out of glbparser's anonymous namespace). Quadric-error half-edge collapse over a welded triangle list + the tier policy both bakes use (`kRatios` 0.5/0.25 of the welded count, `kMinCorners`, `kShrinkSlack`). **The one trap:** `weld(..., keyNormals)` must be FALSE for static meshes - their normals are DERIVED (face normals, then `meshlod::smoothNormals`' 60-degree crease smoothing, which is where a low-poly pole's shading gradient comes from - `objparser::load` runs it so viewport and .tmdl agree by construction), so keying on them locks crease corners as seam twins and little decimates (`generateTiers` welds by position+uv and re-derives per tier: `recomputeFaceNormals`, then `smoothNormals`, so a LOD switch does not pop back to flat). Animated meshes keep `keyNormals` on: authored smooth normals are real data and a hard edge must stay a seam. |
