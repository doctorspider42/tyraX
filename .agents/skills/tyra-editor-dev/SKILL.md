...
The game is compiled by native PS2DEV plus vendored OpenVCL by default; Docker
remains a selectable fallback. PCSX2 launches the resulting ELF.
...
| `staticbatch.{hpp,cpp}` + `staticbatchdisk.cpp` + `batch_ui.cpp` | ~640 + ~160 + ~470 | **Seeing how static objects batch** (docs/static-batching.md) - Tools > Static Batches, the viewport overlay and `--batch-report`. **A TWIN BY NECESSITY**: the grouping is generated code (`TerrainGame::buildStaticBatchList`) that runs on the EE and reads loaded models/materials, `g_dynLights` and the engine's `Texture*` pointers, so moving it host-side would mean baking a batch table into scene_data.hpp and changing every project's generated output. This is the host implementation of the same rules (scrollsim/livelogic arrangement), host-only - the disk-backed asset supplier is a separate TU so the twin links alone in the harness. The oracle is `examples/vehicle-playground/authoring/verify-batch-twins.py`, which lifts buildStaticBatchList verbatim and diffs the assignment member for member (verify-road-twins.py arrangement); the eligibility half is oracled free by `batchStatic` in any generated scene_data.hpp. **The trap**: `acquireTexture` returns nullptr for a missing file and a bag groups by that pointer, so every missing texture batches with every untextured primitive - fixture E pins it, and 'fixing' it makes the panel wrong. `Reason` is the two-stage taxonomy (build vs runtime) and must stay derived from the code: a new `staticBatchEligible` rule needs a `Reason` or the panel names the wrong cause. |
...
| `shadowbake.cpp/.hpp` | ~800 | **Baked shadow decals** (docs/shadows.md): for every object with `shadowMode == 4` the host traces the shadow it throws along the scene's sun into a tile, packs the tiles into shared 256x256 atlas pages and projects them onto the receivers with `decalproj`. Host-only, no GL - the aobake/gibake shape. **THE ATLAS IS WHAT MAKES IT AFFORDABLE, not a VRAM tweak**: a bag is one texture, so one page merges a layer's shadows into ONE submit against ~1 ms of EE each unmerged (docs/prefabs.md). Folding a tile's rect is free at run time - `decalproj` clips to the unit cube so its UVs are in [0,1] by construction. Four rules: the direction is `project::resolvedSettings().lightDir` (the same sun gibake reads, at `ambience::bakedHour`); the receiver search uses the SCENE tree while the shadow rays use a CASTER-ONLY tree (scene occlusion would make two neighbouring casters bake each other's shadow and darken the ground twice), starting past the caster's own extent so a crate does not catch its own bottom face; the tile is laid out in the basis read BACK from `decalproj::projectorBasis`, because at gimbal lock the Euler extraction returns a different valid frame; and `strength` rides the ALPHA, never the tint, so 0 means no shadow. Deterministic per texel, content-hashed cache in `.res-baked/shadow/` (checked into git like gibake's), pages written by texbake from that one cache. **No GPU backend, by measurement** - ~24 rays at one caster's twelve triangles against GI's 128 at the whole tree; `--bake-shadows` prints its wall clock (0.02 s on a two-caster fixture). |
| `occlusionbake.cpp/.hpp` | ~300 | **Conservative software occlusion** (docs/occlusion-culling.md). Host code voxelises only closed, proved-opaque static OBJs and greedily merges eroded interior cells into at most 32 inner boxes; repeated assets share the calculation. Codegen emits those boxes plus per-object receiver/occluder flags in `occlusion_data.gen.hpp`. The generated runtime projects each box as one convex hull into a 48x42 CPU depth buffer, erodes coverage, expands candidate AABBs and rejects whole solo objects, batches, road chunks and procedural chunks only when every cell is safely behind. Near-plane/screen-edge uncertainty and occluders themselves always stay visible. Keep the two per-object opt-outs independent: transparent or authored-hole geometry must not occlude, while an unusual object may separately refuse being culled. |
