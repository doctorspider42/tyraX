...
| `eeexception.cpp/.hpp` | ~180 | Pure host decoder for ps2link's EE/IOP register dump. It accepts both the old raw `Cause ... BadAddr/BadVAddr ... EPC ...` line and the r7 output, returns the stable `TXE-<CPU>-<four-digit ExcCode>` ID plus human diagnosis, and is polled from `devkit_ui.cpp` so a vector the engine crash handler cannot own still opens the Debugger. The numeric suffix is the exact MIPS `Cause.ExcCode`; null/near-null is only a `BadVAddr < 0x10000` diagnosis and must never become a different ID. Keep its names and descriptions in lockstep with `CrashHandler::causeDescription`, the ps2link patch and docs/devkit.md. |
