...
| `footik.{hpp,cpp}` | ~700 | **Foot IK, host side** (docs/foot-ik.md). Host-only, no GL - the aobake/placement shape. Three jobs kept apart: AUTHORING (`autoDetect` guesses leg chains from bone names - Mixamo/Rigify/UE/plain - settling the one genuinely ambiguous case with the HIERARCHY, since a bare "...Leg" is the shin in Mixamo and the thigh elsewhere; `resolve` turns names into the indices the console binds by and refuses a chain the hierarchy does not connect or a pelvis that is not an ancestor of every hip; `measureSoleOffset` reads the number off the bind-pose mesh instead of guessing), VERIFICATION (`evalPose`/`solve` are twins of the engine's `SkelInstance::evalPose` and `FootIk` - **change one, change both**), and nothing else. **The scaling rule that cost a bug:** the struct defaults are authored for a ~1.8-unit human, so `autoDetect` scales every LENGTH by the model's own height - forgetting that put a 3.4-unit character's feet in the air on stairs. `solve`'s knee-side choice is a POLE test (which way the knee points off the hip-to-ankle line), never a comparison against the thigh: the thigh version ties on a big swing and flips the knee ~100 degrees between frames, while the pole version can only tie when both candidates are the same vector. |
| `footik_ui.cpp` | ~380 | The Foot IK window (Tools > Foot IK) - App:: methods declared in app.hpp, own TU (the credits_ui.cpp precedent). Separate from the Animation Editor because a rig belongs to the SKELETON and a clip edit to one clip. Widgets only; everything with judgement in it is in footik.cpp. Carries the `sectionJson` guard over `Section::AnimRigs`. |
| `motionnet.{hpp,cpp}` | ~600 | **The gait net, host side** (docs/neural-gait.md). The SINGLE SOURCE of three things that must agree or the net reads garbage: the feature layout (mirrored by the engine's `motion_net.cpp`), the joint ORDER the outputs map onto (`outputJoints`, carried in the .tnet so the runtime never derives it twice), and the .tnet binary layout. `generateDataset` walks the character over seeded procedural terrain and emits what the IK SOLVER produced as targets - not circular, because the solver sees only the ground under the foot now while the net's inputs carry the ground ahead, so distilling one into the other with a wider input is what buys anticipation; the phase-rate target is a stated rule instead, since no solver has an opinion about stride length. `tools/motion-net/train.py` knows none of the three - it reads a CSV this file wrote and writes a weights JSON this file validates and bakes. |
