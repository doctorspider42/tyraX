...
| `footik.cpp/.hpp` | ~200 | **Foot IK authoring, host side** (docs/foot-ik.md). Host-only - no GL, no ImGui, no `App` (the placement/aobake shape), so bone auto-detection and rig validation run from a 40-line harness against a real skeleton instead of only from a docked panel. It takes the skeleton as two plain arrays (names + parent indices), which is what both the .glb/.fbx info cache and a harness can hand it. `autoDetect` recognises the conventions that turn up (Mixamo, Blender Rigify, Unreal, plain English, the numbered `Leg_Left_1..3` style) by comparing names with case, punctuation and separators removed, and deliberately leaves `enabled` OFF - a guess is a starting point the author confirms. `validate` reports WHICH bone is wrong (missing, duplicated in the file, not a real hip > knee > ankle chain, the same bone in two slots) rather than silently dropping a leg; `findNode` returns -2 for an ambiguous name because the console binds by name and could retarget to the other leg on the next export. **The SOLVER is deliberately not here**: it lives in the generated game (`templates.cpp::applyFootIk`) and the editor does not carry a second copy - a twin nobody could see drifting is worse than no twin, which is why the viewport keeps showing the authored clip. `slotField`/`slotLabel` are the one place the six slots are named, so the tool renders six identical rows. |
| `footik_ui.cpp` | ~430 | **Tools > Foot IK** - App:: methods declared in app.hpp, own TU (the credits_ui.cpp/facts_ui.cpp precedent). The window's SHAPE is the feature's design: a model picker at the top, the shared rig in the middle (Rig / Tuning / Clips / Neural assist tabs), and the list of scene objects using that model at the bottom - everything a crowd shares is authored once, and who pays for it is the column of checkboxes at the end. `footIkRigFor` creates a rig on first touch and seeds its distances from `unitsPerMeter` (project::create's rule: a metric default must mean the same thing at any world scale). Carries the `sectionJson` guard across the whole body (app.hpp rule 1) because the per-widget `changed` flag is what the next widget forgets; the Instances tab edits SCENES, which the undo snapshot covers on its own. A bone Selectable is suffixed with its node index - a skeleton may legitimately repeat a name, and a Selectable's label IS its ImGui id. |
| `glbparser.cpp/.hpp` Foot IK contract | `.tskl` v3 preserves every skeleton node's complete author name as a length-prefixed string; v4 additionally bakes each clip's authored gait speed from the horizontal sweep of both auto-detected ankles. The engine loader remains compatible with v1-v3 (missing gait speed = legacy locomotion fallback) and rejects ambiguous duplicate-name lookup. **The rig is per MODEL ASSET and the switch is per INSTANCE** - `Project::footIkRigs` (`Section::FootIkRigs`, one `FootIkRig` per animated model: six bone names, the tolerances, the descent reach, the neural settings and a list of per-clip rules) plus a plain `SceneObject::footIk` bool. That split is the whole point: a skeleton cannot be two things, a crowd shares one binding, and a future NPC opts in without re-authoring anything. Codegen emits `FOOT_IK_RIGS` indexed by `animModel` (model_data.gen.hpp, next to ANIM_MODEL_PATHS) with the per-clip rules flattened into `FOOT_IK_CLIP_RULES` sliced per rig (the CATCH_CANDIDATES arrangement), and `footIkRigOf` is the ONE answer to "can this instance solve" - read by both the scene-table switch and the rig row, so an emitted `footIk = 1` always has a rig behind it. A clip rule is resolved at runtime BY NAME but only when `RuntimeObject::animClip` changes: a per-frame string compare per instance would be paid forever for an answer that almost never moves. A rule that says "do not solve" drops the contacts (a jump must not land the solver back on the stair it left) but KEEPS the locomotion filters, which are about the object and not the pose. Rules are stored under the SOURCE clip name (the AnimClipEdit convention) and translated to effective names at codegen, so an Animation Editor rename cannot orphan one. A pre-v12 file carried both halves on the object; the lift is a LOAD-TIME shim in `readObjectsArray` + the tail of `load()`, not a migrations.cpp step, because the move is unambiguous - two objects that shared a model but disagreed about its bones keep the first binding and say so on stderr. `templates.cpp::applyFootIk` turns the previous frame's **unadjusted clip** sole probes into the engine's generic `SkelPoseAdjust` (feeding the adjusted/locked pose back makes a plant impossible to release). Its contact policy has THREE gates: the ordinary narrow locomotion band, a larger `plant + 55% pelvis + descent reach` capture available only while the foot descends (or the BODY does) and slows horizontally into stance, and a 0.1-second whole-object-still rest contact which may reach down by the full plant + pelvis range. Moving contacts also require a short stable-candidate window; a fast airborne swing must never plant merely because it crossed the vertical band. New contacts use a 0.14-second smoothstep envelope. Toe-off releases when filtered upward motion and horizontal sole speed agree, and preserves the old target while the weight smoothsteps out over 0.08 seconds, so target height and influence cannot jump together; rest contact releases the instant the object moves. Merging the direction gates makes swing feet sticky. The swept footprint contributes clearance only for support more than 2.5 cm above the current plane; applying its shoe margin to equal-height ground manufactures flat-ground hover. Pelvis correction has its own low-pass state so an edge probe changing surfaces does not shake the whole avatar. A filtered authored knee pole is weighted strongly and the engine promotes it near full extension, where the analytic bend plane is singular. `TYRAX_FOOT_IK_TRACE=1` is a generator-only regression switch which emits `PLAYERIK`/`FOOTIK`/`FOOTTRAIN` CSV-shaped rows for the testing runner and compiles out of normal builds; `TYRAX_FOOT_NEURAL_EE=1` swaps in the scalar twin for parity runs. The generated runtime owns that policy; `SkelInstance` is only the model-local analytic solver. Foot-IK instances cannot use pose sharing. |
...
Foot IK surface-following extends that contract with two authorable fields:
`maxFootAngle` limits planted ankle pitch/roll toward the raycast normal, and
`toeClearance` lifts an unlocked rising swing over two ahead samples. Their
rise is measured above the plane predicted by the current support normal, so a
continuous slope does not masquerade as a stair lip. A higher abrupt surface
cancels a stale lower-step release target before the vertical riser can
intersect the shoe. Terrain normals use height gradients; mesh hits use
`CollisionMesh::raycast`'s optional triangle normal.

**Walking DOWN is a separate mechanism, and it is the one to understand before
touching any of the bands.** They are tuned for level ground and are
deliberately narrow - a wide band turns every airborne swing into a ground
magnet - but a descending foot must cover the step height PLUS whatever the
avatar's visual root still owes its collision height, which on a 22 cm stair is
already twice the plant distance. No level-ground number stretches to cover
that. So `FootIkRuntime` classifies its own **locomotion grade** from three
signals that are each wrong somewhere on their own: the visual root's vertical
speed against its horizontal travel (silent while the root filter is catching
up, and says "down" during a jump), the drop the walker PUBLISHES
(`RuntimeObject::ikGroundDrop`, exact but walker-only), and a probe under the
path ahead with a speed-proportional lead (works for any mover and sees the step
before it is stepped on). Geometry outranks velocity, ascend and descend are
made mutually exclusive, and the classification is SOFT and filtered because
every gate it widens would otherwise pop on the frame it flipped. What a descent
earns is a BUDGET capped by `FootIkRig::descendReach`, spent in exactly four
places - the probe window, the descending-plant band, the down-reach envelope
and the pelvis's DOWNWARD limit only (a fraction of it: the leg does the work,
and a pelvis allowed to sink a whole step reads as a crouch). Every one stays
raycast-gated: a budget widens where a foot may LOOK, never what it may believe.
Two gates additionally accept the body's grade in place of the swing foot's own
vertical velocity, because in a flat walk clip played by a falling root the
ankle rises in model space while the shoe drops toward the tread. And "there is
a lower tread" no longer requires the OTHER foot to be planted above it - on a
staircase that was a deadlock, with both feet hovering and neither able to be
the support that lets the other reach.

Third-person `PlayerCtl` keeps collision `y` separate from a filtered
`visualY`: collision steps immediately, while the avatar root and camera ease
across grounded height changes; airborne motion follows physics and the filter
resumes on contact. **The ease is asymmetric on purpose** - 16/s down against
8/s up. Filtering a fall as slowly as a climb left the whole avatar hanging
above the tread for several frames, which is most of the gap the feet were then
asked to cover; a rising step still needs the slow filter or the camera hops.
The residual is published rather than guessed, so the two mechanisms cannot
disagree about how far down the ground is.

The optional neural layer is a deterministic 20-16-6 VU0-macro network trained
by `tools/train-foot-neural.py` from synthetic geometry plus
`tools/data/foot-neural-real.csv`. It consumes the near/far lip residuals and
support normal, filtered gap/vertical velocity, decaying lip memory, swing
phase - and the locomotion grade (descend/ascend confidence, filtered drop
ahead, earned budget), which is what lets the heads specialize on a descent
instead of inferring one from a filtered velocity. Its residual seeds a
five-point raycast fan scored for toe/heel support; its clearance/release heads
may only act after procedural raycasts or the four-corner swept sole prove a
lip, and its reach head may only firm up a down-reach envelope a ray already
opened, by at most a quarter. Whole-body pelvis shift/lean and knee pole hints
are applied before the two analytic legs re-establish contacts.
`FOOTTRAIN` telemetry plus the Foot IK regression runner is the single path for
recording and labelling new real trajectories; **runtime normalization, runner
labels and trainer feature order are exact twins** - a divisor changed on one
side alone silently feeds the network a different world than it was trained on.
A recording made before a feature existed still loads: missing columns read as
0, which for the grade lanes means level ground.

**Reading a model is not baking it, and the difference is measured in seconds.**
`animimport::bake` samples every clip into CPU vertex FRAMES; `parseSkel` reads
the hierarchy, the tracks and the bind-pose parts. Both take an opt-out now,
because an animation-library character (one .fbx, forty clips) made the editor
unusable around it: `parseSkel(..., metadataOnly)` skips the channel resampling
AND the all-clips pose AABB (which evaluates the whole scene at 6 Hz per clip -
the single most expensive thing in either parser), and `bake(..., onlyClips)`
samples only the named clips while every other clip keeps a valid Clip entry
holding its first frame. `App::glbInfo` is metadata-only (it wants names, counts,
materials and bones - it was paying for a full bake and throwing the frames
away), and `Viewport::animModelDraw` bakes NO clips up front, with
`ensureAnimClip` sampling one the first time something poses it. Measured on a
39-clip ~5000-vertex rig, release build: selecting the character in Properties
23.5 s -> 5.8 s, and the lazy viewport bake is pixel-identical to the eager one
(checked by flipping that one argument back and diffing the settled frame). Two
traps: a re-bake must keep the GL meshes and decoded textures the first bake
built (only the CPU frame arrays change - bail out rather than swap in a bake
whose part/vertex counts differ), and the sampler has to run BEFORE frame
indices are resolved, because an unsampled clip resolves to one frozen frame and
previews as a static pose instead of failing. Never hand `metadataOnly` to the
.tskl bake: the runtime culls with that box.

**A standing character must get no swing clearance, and this cost two wrong diagnoses before the trace settled it.** The clearance probes need a direction of travel; `travelSpeed` fell back to the ANKLE's speed when the object was still (for an in-place animation), and an idle shuffle - or the turn the avatar makes when the stick is released - pushes that past 0.12. "Ahead" is then whatever the shuffle pointed at, so standing beside a step the probes find its riser, call it a lip and lift the free foot over it FOREVER, because standing still never resolves. It presents as "the foot will not plant" while the solver is in fact lifting it on purpose, and the tell in the trace is the IK TARGET sitting above the foot's own sole (`target_y > sole_y` with weight 0). The fallback is now gated on `objectStillTime >= 0.10F` - the same signal rest contact uses. Measured beside a 22 cm step, one binary, one line: 6 consecutive lifted frames (target 8 cm above the sole) to none. The two dead ends are worth knowing because both looked reasonable: gating the plant on a fraction of the foot's own stride peak instead of body speed made the descent metric WORSE (3% -> 10% unsupported) and rested on a false reading - a sole whose p90 is 2x the body's is normal gait kinematics, not skating - and giving the pelvis only the part of the correction a leg cannot absorb was aimed at a straddle pose that turned out not to be the reported bug at all. **Read `target_y` against `ground_y` AND against `sole_y` before concluding anything about a foot in the air.**

**A part's stored vertex positions are NOT necessarily in the same space as the composed node hierarchy, and that cost two wrong numbers in a row while measuring the Foot IK sole offset.** The obvious formulation - "the lowest vertex weighted to the foot, against the ankle's height" - answered **0.227** on a rig whose ankle really stands at **0.104**, because on the FBX path `SkelPart::positions` are pre-skin control points and the palette's inverse bind matrix is what places them; the .glb path happens to store them already placed, which is why the assumption survived. The tell was printing the numbers side by side: the lowest "foot" vertex sat at -0.123 while the model's own bounds were 0.000 .. 1.829, i.e. below its own floor. Before that, composing bind heights by summing LOCAL translations (rather than full 4x4, rotation included) answered 1.031 - most of a leg - because every Unreal-style rig has a rotated bind pose. `footik::bindHeights` composes properly and `measuredSoleOffset` now needs no mesh at all: it takes the ANKLE's own bind height above the model's floor, which validated at 0.103 against an authored 0.100. The general lesson for anything that measures a model: print the number next to something you already know to be true (the model's bounds, its height) before letting it write a field - a measurement that can be silently wrong by 2x is worse than no measurement.

The authored `SceneObjectData::footIk` bit is the immutable capability; the
generated `RuntimeObject::footIkEnabled` bit is its live gate, shared by scripts,
compiled flow graphs and the Live Logic `SetFootIk` opcode. The disabled branch
must clear both `SkelPoseAdjust` and `ObjectGeometry::FootIkRuntime`, otherwise a
later enable can resurrect a planted target from the wrong stair.
