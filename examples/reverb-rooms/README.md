...
- **CIRCLE** — the *same knock, forced dry*. Stand in the hall and alternate:
  CROSS rings, CIRCLE does not. That is the **Dry** parameter on the Play
  Sound node — for a UI beep or a voice line that has to sound the same
  everywhere.
...
the free one, loads its preset there while it is silent, and ramps the two
past each other. No dry gap.

Two audible consequences. A sound **finishes in the room it started in** —
knock as you step out of the hall and the tail follows you, because a reverb
unit is per SPU2 core and the voice is already on the other one. And only
**two** rooms can be live at once, so sprinting through three
differently-flavoured rooms inside half a second makes the third wait for the
first to finish leaving; it waits rather than glitching.
...
- **`zone-closet`** — preset **Off**, **Priority 1**, sitting inside
  `zone-chamber`. Overlapping zones do not mix; the highest priority
  containing the listener wins, so the closet cuts a dry pocket out of the
  room around it.
- **`hall-drip`** / **`chamber-tick`** — sound emitters, identical but for
  the *Reverb* checkbox.
...
slabs (collision off) and plain box walls. The volumes are what the game
reads — you could move them somewhere else entirely.
...
So: the zone, the *Dry* parameter, the emitter checkbox and the override node
all work — against one unchanging sample.
