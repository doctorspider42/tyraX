  ...
  void update(ScriptContext& ctx) override {
    ...
    if (livedbg::forced(0)) {  // Live Debugger: fired from the editor
      ...
      {  // Sequence output 1
        livedbg::hit(2);
        {
          const float v = 30.0F;  // "health"
          ctx.saveValues[0] = v;  // "health"
          ctx.hudBarValue[0] = v;
          ctx.hudBarSet[0] = 1;
        }
      }
      {  // Sequence output 2
        livedbg::hit(3);
        ctx.hudElemEffect[2] = 1;  // "health"
        ctx.hudElemEffectSec[2] = 0.45F;
      }
    }
    ...
    if (livedbg::forced(8)) {  // Live Debugger: fired from the editor
      livedbg::hit(8);
      livedbg::hit(9);
      ctx.hudElemRequest[3] = 2;  // "stamina" (bar)
    }
    ...
    if (livedbg::forced(10)) {  // Live Debugger: fired from the editor
      livedbg::hit(10);
      livedbg::hit(11);
      ctx.hudElemEffect[0] = 2;  // "ANIMATED HUD"
      ctx.hudElemEffectSec[0] = 0.55F;
    }
...
TYRA_SCRIPT(Hud_animation::FlowGraphScript_0_0);
