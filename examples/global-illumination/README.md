...
open to a daylight sky. Nothing in it is coloured except those two walls;
everything else is white or grey. So every tint you see anywhere else came off
a wall — which is the whole point of
...
The frame holds **50 FPS** (full PAL) with GI on — the console does no extra
work at all. The lighting is a texture and a table; the ray tracing happened on
your desktop.
...
ambient shading, no bleed, no shadows beyond the analytic contact term. Tick it
back on and press **Bake this scene** to get the light back (about 10 seconds
for this scene).
...
project with GI quietly switched off. The bake signature hashes file **content**,
not timestamps, so a checkout does not invalidate it.

Edit anything — move a wall, repaint it, drag the sun — and the bake goes
**stale**: the Bake window says so per scene, and the build falls the scene back
to the classic lighting until you re-bake. That is deliberate: stale is visible,
never silently shipped.

## Controls

Standard FPP template: left stick walks, right stick looks, X jumps. Walk into
the alcove and back out into the open to feel the probe grid work on the player's
surroundings.
