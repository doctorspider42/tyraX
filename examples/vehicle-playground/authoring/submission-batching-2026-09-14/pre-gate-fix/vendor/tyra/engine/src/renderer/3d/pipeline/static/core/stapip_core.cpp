...
void StaPipCore::render(StaPipBag* bag) {
  ...
  const bool residentTexture =
      !batchScope || !hasTexture ||
  ...
  qbufferRenderer.setSubmissionBatchCandidate(
      batchScope && directBag && residentTexture && repeatTexture &&
          bag->billboard == nullptr && bag->count <= maxVertCount * 15 &&
          !qbufferRenderer.hasProgramOverrideForBag(bag),
      hasTexture);
