...
constexpr u16 kQBufferCommandQwords = 8;
constexpr u16 kHalfBufferPacketSize =
    (kQBufferCommandCapacity / 2) * kQBufferCommandQwords + 1;
constexpr u16 kBagAdmissionSize =
    kObjectDataPacketSize + kHalfBufferPacketSize;
}
...
StaPipQBufferRenderer::StaPipQBufferRenderer() {
  ...
  packetSize = kSubmissionBatchSize * kBagAdmissionSize;
...
void StaPipQBufferRenderer::flushBuffers() {
  ...
  if (!is1stDBuffer && !is2ndDBuffer) {
    ...
    if (submissionBatchCandidate)
      ensurePacketCapacity(size * kQBufferCommandQwords + 1);
  ...
  if (submissionBatchCandidate && currentBatchBagHasGeometry) {
    ++submissionBatchBags;
    if (submissionBatchBags >= kSubmissionBatchSize) flushPendingPacket();
  }
...
void StaPipQBufferRenderer::cull(StaPipQBuffer* buffer) {
  ...
  if (is1stDBuffer || is2ndDBuffer) {
    ...
    if (submissionBatchCandidate) {
      ensurePacketCapacity(kHalfBufferPacketSize);
      addBuffersDataToPacket(from, to, false);
    } else {
...
void StaPipQBufferRenderer::addBuffersDataToPacket(const u32& from,
  ...
  if (added) {
    objectDataPending = true;
    currentBatchBagHasGeometry = true;
  }
...
void StaPipQBufferRenderer::setSubmissionBatchCandidate(const bool& candidate,
  ...
  // Modified by TyraX: reserve worst-case uniforms plus the first 16-group
  // half before appending any bytes. Later halves check their own capacity.
  // Checking after construction is too late: packet2 would already have
  // written past base.
  if (enabled && objectDataPending &&
      packet2_get_qw_count(packets[context]) + kBagAdmissionSize >
          packetSize)
    flushPendingPacket();
  submissionBatchCandidate = enabled;
  currentBatchBagHasGeometry = false;
  currentBatchBagTextured = enabled && textured;
  if (enabled) submissionPacketIsBatch = true;
  if (currentBatchBagTextured) submissionPacketHasTexture = true;
  if (enabled && telemetry) ++telemetry->submissionBatchEligibleBags;
}

void StaPipQBufferRenderer::ensurePacketCapacity(const u16& requiredQwords) {
  if (!objectDataPending ||
      packet2_get_qw_count(packets[context]) + requiredQwords <= packetSize)
    return;
  if (telemetry && currentBatchBagHasGeometry)
    ++telemetry->submissionBatchSplitPackets;
  flushPendingPacket();
  // This is a geometry-only continuation of the current bag. Its absolute
  // uniforms remain in VU memory, but texture lifetime tracking must follow
  // every packet that starts more draws from those uniforms.
  submissionPacketIsBatch = true;
  if (currentBatchBagTextured) submissionPacketHasTexture = true;
}

void StaPipQBufferRenderer::flushPendingPacket() {
  if (!objectDataPending) return;
  if (!currentBatchBagHasGeometry && submissionBatchBags == 0) {
  ...
  if (telemetry && submissionPacketIsBatch) {
...
bool StaPipQBufferRenderer::hasProgramOverrideForBag(const StaPipBag* bag) {
  return repository.hasOverride(getCullProgramByBag(bag)->getName());
}
