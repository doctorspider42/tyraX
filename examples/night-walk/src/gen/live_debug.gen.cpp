...
// a GS frame buffer back over the VIF1 reverse FIFO and writes it as a TGA.
// This is what lets the game photograph ITSELF - see writeFrameCapture below.
...
const unsigned int HASH_LO = 1053844838U, HASH_HI = 953960947U;
...
// out of GS VRAM over the VIF1 reverse FIFO and writes an uncompressed 32-bit
// TGA. Every host-side capture path (PCSX2's F8, GDI, PrintWindow) needs a
// desktop that is unlocked and a window that is on top, and NONE of them exists
// on real hardware - this one works on both, which is the whole point.
//
// Four things decide whether it produces a picture at all, and each of them
// fails SILENTLY - which is why they are written down rather than trusted:
//
...
//  * libdebug reads a LINE at a time through static 1024-word buffers, so a
//    frame buffer wider than 1024 pixels would overrun them. No display mode
//    this engine offers comes near that, but the guard costs one comparison.
//  * IT OPENS THE FILE `O_CREAT|O_WRONLY` WITH NO `O_TRUNC`. A shorter capture
//    written over a longer one therefore leaves the previous picture's tail
//    behind, and the file still decodes - as this frame with somebody else's
//    rows under it. Deleting first is the whole fix, and it is also what makes
//    the size check below mean something.
//
// And the return value is NOT a verdict: ps2_screenshot_file returns 0 both
// when open() fails and when everything worked (upstream has no `return 1` on
// the success path at all). So the honest check is to measure the file we just
// asked for - 18 bytes of TGA header plus 4 bytes per pixel - and report THAT.
void writeFrameCapture(ScriptContext& ctx) {
  if (!ctx.engine) return;
  framebuffer_t* fb = ctx.engine->renderer.core.gs.getPreviousRealFrameBuffer();
  if (!fb || fb->width == 0 || fb->height == 0) return;
  if (fb->width > 1024) {
    TYRA_WARN("Frame capture: ", fb->width, " px is wider than libdebug's "
              "line buffer - skipped");
    return;
  }
  const std::string path = Tyra::FileUtils::fromCwd("frame.tga");
  remove(path.c_str());
  ps2_screenshot_file(path.c_str(), fb->address / 64, fb->width, fb->height,
                      fb->psm);
  const unsigned int want = 18U + fb->width * fb->height * 4U;
  unsigned int got = 0;
  if (FILE* f = fopen(path.c_str(), "rb")) {
    fseek(f, 0, SEEK_END);
    got = (unsigned int)ftell(f);
    fclose(f);
  }
  if (got == want)
    TYRA_LOG("Frame capture: ", fb->width, "x", fb->height, " at frame ",
             frameNo, " written to frame.tga");
  else
    TYRA_WARN("Frame capture: frame.tga is ", got, " bytes, expected ", want,
              " - the host: write did not complete");
}
