 ...
 public:
  void update(ScriptContext& ctx) override {
    ...
    if (livedbg::forced(0)) {  // Live Debugger: fired from the editor
      ...
      if (!flip2) {
        livedbg::hit(2);
        ctx.objects[0].footIkEnabled = false;
        livedbg::hit(4);
        TYRA_LOG("Foot IK: OFF (raw animation)");
      } else {
        livedbg::hit(3);
        ctx.objects[0].footIkEnabled = true;
        livedbg::hit(5);
        TYRA_LOG("Foot IK: ON (terrain solver)");
      }
      flip2 = !flip2;
    }
    ...
    if (livedbg::forced(6)) {  // Live Debugger: fired from the editor
      livedbg::hit(6);
      livedbg::hit(7);
      TYRA_LOG("Foot IK A/B: press L1 to toggle");
    }
    if (!started) {
      started = true;
      livedbg::hit(6);
...
TYRA_SCRIPT(Foot_ik_stairs::FlowGraphScript_0_0);
