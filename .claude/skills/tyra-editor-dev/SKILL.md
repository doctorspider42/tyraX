...
| `mhdata.cpp/.hpp` | ~330 | Readers for the MakeHuman **CC0 data** in `vendor/mh-assets` (fetched by setup.ps1 from the `$MhAssets` list in deps.ps1 - DATA only; the MakeHuman program is AGPL and none of it is used). Host-only, no GL: the base `.obj` (quads, groups), `.target` sparse morph deltas, `.proxy` barycentric bindings, `.mhskel` (bones + joints-as-vertex-cubes), `.mhw` vertex weights. Everything is in MakeHuman's space - **decimetres, origin at the hips** - and converting is chargen's job, done once at the end. Targets and proxies index the FULL base vertex array, so never compact it before morphing. The `.proxy` **one-token short form** (a proxy vertex sitting exactly on a base vertex) must be honored: dropping it shifts every later vertex against the .obj's faces and scrambles the mesh subtly instead of failing. |
