...
| `aobake.cpp/.hpp` | ~380 | **Baked ambient occlusion** (docs/ambient-occlusion.md). Host-only, no GL - the decalproj pattern. `terrainAO` (heightmap horizon scan → u8 grid; codegen ships it as `TERRAIN_AO_TABLES`, the viewport multiplies the SAME grid into its terrain colors), `collectOccluders` (solid objects → oriented-box/sphere shapes - the single source for codegen's `ao_data.gen.hpp` AND the viewport shader uniforms), `modelAO` (deterministic raycast self-AO per obj position; texbake writes the `<model>.aov` sidecar the engine's LeanObjLoader reads). The occlusion RESPONSE formula is a twin: generated game `aoOccluderAt`/`aoShadeMul` (per vertex at scene load, in pushVert/shadeAt before point lights) ⇄ viewport fragment shader `aoOcclusion` (per fragment, live) - change one, change both. Settings: `aoEnabled/aoStrength/aoRadius/aoTextured` on ProjectSettings + AmbiencePreset (Ambience Editor UI); per-scene via the ambience preset like the rest of lighting. The experimental **Textured** quality (`aoTextured`) bakes the same occlusion into per-pixel textures instead: `terrainAOMap` (terrain AO map) + `bakeSceneAoAtlas` (per-scene primitive lightmap atlas; regions mirror the builders' UV layouts - box 6/sphere 1/cylinder 3/cone 2/plane 2 - and pushVert emits atlas STs via `g_aoRegion`), drawn as extra alpha-blended passes (black RGBA32 texture + GS alpha-over = per-pixel multiply; **palettized alpha loses the gradient in the engine's tRNS→CLUT path — keep these RGBA32**). texbake writes the PNGs (`.res-baked/aomap|aoatlas/`), codegen the rects, from the same deterministic bake. Covered objects leave static batching. |
