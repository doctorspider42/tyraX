...
mandatory `.desc`) → a branch in `evalNode` (procgen.cpp) → nothing else: the
window renders params from the registry, and the bake only sees the Output.
...
than a reseed. A node that reads the scene (objects, terrain) must have its
