...
| `weaponedit.cpp` | ~470 | **Weapon Editor** (Tools > Weapon Editor, docs/weapons.md) - App:: methods declared in app.hpp, own TU like assetbrowser.cpp. Weapons are project-wide (`Project::weapons`), edited through `saveAll` and NOT on the undo stack, like fonts/menus/sequences. Also owns `weaponCombo` (the flow-node picker - `allowEmpty` is not cosmetic: several Combat nodes read "" as "the equipped weapon"/"any weapon"), `renameWeaponRefs` (a weapon name is referenced from object loadouts AND `WeaponName` node params - both must follow a rename) and `addWeaponModelToScene` (weapongen -> `.obj` -> a Model object -> the weapon's `viewModel`, one click). The **Viewmodel preview** (`drawWeaponViewmodelPreview` + `Viewport::renderWeaponPreview`) is the one preview in the editor whose default camera is not a turntable: a viewmodel is authored FROM THE EYE, so it renders at the origin looking down +Z at the engine's 60-degree vertical fov and the project's aspect - an *Offset* is a place on the SCREEN, and a turntable cannot show that. The panel resolves the pose and the viewport only renders a transform, which is what keeps `weaponPreviewPose` **a twin of `wpnPinViewModels`** (templates.cpp) rather than a second renderer: sway/bob/kick/reload/swing are duplicated there and BOTH must move together, or the preview quietly stops predicting the console. The eye basis collapses the runtime's to right = (-1,0,0), which is why an Offset X of +1 lands at world -1. |
...
as a real edit and puts the full rebuild back.
