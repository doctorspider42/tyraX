...
| `glbparser.cpp/.hpp` Foot IK contract | `.tskl` v3 preserves every skeleton node's complete author name as a length-prefixed string; v4 additionally bakes each clip's authored gait speed from the horizontal sweep of both auto-detected ankles. The engine loader remains compatible with v1-v3 (missing gait speed = legacy locomotion fallback) and rejects ambiguous duplicate-name lookup. `SceneObject::footIk` stores two hip/knee/ankle mappings plus probe/plant/pelvis tuning, `props_ui.cpp::drawFootIk` validates and auto-detects common humanoid names, and `templates.cpp::applyFootIk` turns the previous frame's **unadjusted clip** sole probes into the engine's generic `SkelPoseAdjust` (feeding the adjusted/locked pose back makes a plant impossible to release). Its contact policy has THREE gates: the ordinary narrow locomotion band, a larger `plant + 55% pelvis` capture available only while the foot descends and slows horizontally into stance, and a 0.1-second whole-object-still rest contact which may reach down by the full plant + pelvis range. Moving contacts also require a short stable-candidate window; a fast airborne swing must never plant merely because it crossed the vertical band. New contacts use a 0.14-second smoothstep envelope. Toe-off releases when filtered upward motion and horizontal sole speed agree, and preserves the old target while the weight smoothsteps out over 0.08 seconds, so target height and influence cannot jump together; rest contact releases the instant the object moves. Merging the direction gates makes swing feet sticky. The swept footprint contributes clearance only for support more than 2.5 cm above the current plane; applying its shoe margin to equal-height ground manufactures flat-ground hover. Pelvis correction has its own low-pass state so an edge probe changing surfaces does not shake the whole avatar. A filtered authored knee pole is weighted strongly and the engine promotes it near full extension, where the analytic bend plane is singular. Third-person `PlayerCtl` keeps collision `y` separate from an 8/s-filtered `visualY`: collision steps immediately, while the avatar root and camera ease across grounded height changes; airborne motion follows physics and the filter resumes on contact. The optional neural assist is a reproducibly trained 16-16-5 landing predictor (`tools/train-foot-neural.py`): VU0 macro-mode dense products consume a short motion history and an ahead probe and seeds a five-point raycast fan scored for toe/heel support; every candidate, step height and final IK limit stays collision/procedural. `TYRAX_FOOT_IK_TRACE=1` is a generator-only regression switch which emits `PLAYERIK`/`FOOTIK` CSV-shaped rows for the testing runner and compiles out of normal builds; `TYRAX_FOOT_NEURAL_EE=1` swaps in the scalar twin for parity runs. The generated runtime owns that policy; `SkelInstance` is only the model-local analytic solver. Foot-IK instances cannot use pose sharing. |
...
and support normal plus filtered gap/vertical velocity, decaying lip memory and
swing phase. Its residual seeds a five-point raycast fan scored for toe/heel
support; its clearance/release heads may only act after procedural raycasts or
the four-corner swept sole prove a lip. Whole-body pelvis shift/lean and knee
pole hints are applied before the two analytic legs re-establish contacts.
`FOOTTRAIN` telemetry plus the Foot IK regression runner is the single path for
recording and labelling new real trajectories; keep runtime normalization,
runner labels and trainer feature order exact twins.
