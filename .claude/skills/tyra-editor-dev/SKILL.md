...
| `version.hpp` + `migrations.cpp/.hpp` | ~150 | **Editor/format versioning.** `version.hpp`: the editor semver (title bar + informational `"editorVersion"` in the manifest) and `kFormatVersion`, the on-disk contract (`"formatVersion"`; pre-versioning files = v0). `load()` refuses newer-format files; older ones open silently unless `migrations::stepsFor` returns registered steps — then the GUI (`App::openProjectAt`) prompts, backs up the format-bearing files into `_backup/` and migrates in memory (save only on success), and headless `--build`/`--resave`/`--apply-graph`/`--ai-graph` refuse (use `--migrate`). Rules + step-authoring example: `docs/format-versioning.md`. |
