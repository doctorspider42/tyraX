...
| `droneui.cpp` | ~900 | The Drone Generator window - App:: methods declared in app.hpp, own TU (the assetbrowser.cpp precedent). Owns the widget vocabulary the rest of the editor does not have: the **rotary `knob()`** (vertical drag, Shift fine, double-click resets, `curve` for frequency/time knobs, symmetric ranges auto-drawn bipolar). **A knob's label IS its ImGui ID**, so two knobs with the same caption in one panel (a delay *Mix* and a reverb *Mix*) are an ID conflict that breaks hover/drag - suffix BOTH with `##scope` (`Mix##delay` / `Mix##reverb`); the label is displayed only up to `##`. PROGRESS 188 has the two ways to check this, and why ImGui's own conflict detector cannot be driven with a synthetic cursor, `knobInt`, the arc-envelope editor, the scope/analyzer/meters strip. Rules: every parameter edit ends in `dronePushParams()` (that is what makes the audition live), the render runs on `droneRenderThread_` and is only read after `droneRenderDone_` (Runner idiom), and `droneTickRender()` is what writes the WAV + `.drone` and calls `saveAll` - all pixel literals go through `scaled()`. |
