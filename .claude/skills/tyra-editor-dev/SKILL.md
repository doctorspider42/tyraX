...
| `iso9660.cpp`, `isoexport.cpp` | 396+400 | In-tree ISO9660 writer + disc layout planning (`Project > Export PS2 ISO`, Disc Layout window). `iso9660::Options` can reserve a head/tail for a second filesystem; the defaults reproduce the plain image byte-for-byte. |
| `udf.cpp` | 400 | Read-only UDF 1.02 writer that overlays an ISO9660 image into a UDF/ISO9660 hybrid, for `Project > Export FreeDVDBoot ISO` (boot on a **stock** PS2 — [docs/freedvdboot.md](../../../docs/freedvdboot.md)). It exposes only `VIDEO_TS/`, because that is all the PS2's DVD Player reads; the game still loads its assets off the ISO9660 side. **Trap:** descriptors *inside* the partition (File Set, File Entries, File Identifiers) must stamp their tag with the partition-**relative** block, while volume structures (anchors, VDS, LVID) use the absolute sector. Get it wrong and every CRC still validates while no driver can find the filesystem. |
