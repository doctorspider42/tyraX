...
| `app.cpp/.hpp` | 2751 | The whole ImGui shell: menus, all panels (Project, Scene, Scripts, HUD, Music, Sounds, Output, Disc Layout, Flow Graph) and tool windows (Menu/Save/UI Editor, Color Grading, Ambience, Cutscene Director, Loading Screens), modals, gizmo + sculpt input, undo/redo, clipboard, wiring viewport ↔ project ↔ runner. The **Save Editor** (`drawSaveEditorWindow`) owns the memory-card title/icon, the exact slot-size breakdown (`templates::saveSizeInfo`) and the save values/texts (`drawSaveDataSection`). |
...
| `savebake.cpp/.hpp` | ~230 | Memory card save appearance bake (Tools > Save Editor): `icon.sys` (964 B - browser title with `\|` line break, bg/light colors, icon filenames) and `list.icn` (a PS2 3D icon - static two-sided quad, 128x128 BGR555 texture from `Project::saveIcon` via stb, or a built-in placeholder), written to `res/save/` on every `refreshGenerated` and copied onto the card by the generated save system (`saveEnsureIcons`). Also `busyText()` - the single source for the "checking memory card" overlay, so the baked `res/hud/save-busy.png` and the codegen'd `SAVE_BUSY_W/H` constants can't disagree. Binary formats per Akesson's "PS2 Icon Format v0.5". |
