...
| `shadowbake.cpp/.hpp` | ~800 | **Baked shadow decals** (docs/shadows.md) - the static directional shadow: for every object with `shadowMode == 4` the host traces the shadow it throws along the scene's own sun into a tile, packs the tiles into shared 256x256 atlas pages and projects them onto the receivers with `decalproj`. Host-only, no GL, no ImGui - the aobake/gibake/decalproj shape. **THE ATLAS IS NOT A VRAM OPTIMISATION, IT IS WHAT MAKES THE FEATURE AFFORDABLE**: a bag is one texture, so one shared page is what merges every shadow of a layer into ONE submit, against ~1 ms of EE each unmerged (docs/prefabs.md) - the difference between three hero objects and dozens. Folding a tile's rect costs nothing at run time either, because `decalproj` clips to the projector's unit cube and its UVs are in [0,1] by construction (unlike `texatlas`, which needs a runtime multiply). Four decisions worth keeping. (1) **The direction is `project::resolvedSettings(p, sc).lightDir`** - the same field, already folded through the ambience preset and its day/night cycle at `ambience::bakedHour`, that gibake reads; a second answer to "where is the sun" would put the decal's shadow and the lightmap's in different places. (2) **The receiver search uses the SCENE tree and the shadow rays use a CASTER-ONLY tree.** Occluding with the scene would be the physically fuller answer and is the wrong one: two casters near each other would each bake the other's shadow into their own tile and the overlapping projections would darken the ground twice. The receiver search starts past the caster's own extent along the light (`Caster::casterDepth`), which is also what keeps a crate resting ON the ground from catching its own bottom face and baking a shadow at zero distance, i.e. with no penumbra at all. (3) **The projector is handed to `decalproj` as a synthetic `SceneObject`**, and the tile is laid out in the basis read BACK out of `decalproj::projectorBasis` rather than the one that went in - at gimbal lock `eulerFromBasis` folds the roll into yaw and returns a different (equally valid) frame, and a tile laid out in the other one is mirrored with nothing to point at. (4) **`strength` rides the tile's ALPHA, never the tint**, so 0 means no shadow instead of "replace the surface with a flat patch of sky colour"; the tint is `skyColor * brightness * ambient`, i.e. the surface with the sun taken off it. Determinism is the usual rule - a texel's sample spiral is rotated by a hash of its own coordinates, so the bake is bit-identical at any core count (`cmp` the `.shadow`, never assert it). The cache is `.res-baked/shadow/`, content-hashed like gibake's and checked into git for the same reason; `texbake` writes the pages from that ONE cache codegen reads its UVs out of. **There is deliberately no GPU backend, and that is a measurement**: a shadow texel fires ~24 rays at one caster's twelve-triangle box where a GI texel fires 128 at the whole scene tree, and `--bake-shadows` prints its wall clock (0.02 s on a two-caster fixture) so the decision can be re-run. |
| `cloth.cpp/.hpp` | ~260 | **Cloth / soft bodies** (docs/cloth.md), the host half of a twin and the single definition of `PrimitiveType::Cloth` (21). Host-only - no GL, no ImGui, no `project.hpp` - the treegen/dronegen/placement shape, so the whole solver runs from a 40-line harness; the viewport preview (`Viewport::updateClothPreviews`) and the generated game (`buildCloths`/`updateCloths` in TPL_GAME_CPP_SCENE, written with `Tyra::Vec4` = COP2 macro mode on VU0) are two consumers of it, and the twin must move with it or an authored curtain stops predicting the console. Three properties are the whole design and every change has to keep them. **ONE PARTICLE IS ONE QUADWORD** (`V4`, 16 bytes) and every step is whole-vector arithmetic on it - no per-component code in the hot path, so a line is a VU instruction rather than three scalar ones; the integrator is Verlet precisely because the velocity IS `pos - prev`, so damping is a scale and the acceleration is one madd. **CONSTRAINTS RUN IN FOUR INDEPENDENT BATCHES** (horizontal even columns, horizontal odd, vertical even rows, vertical odd): a distance constraint writes BOTH endpoints, so the naive relaxation pass is a serial dependency chain - the one shape a vector unit is worst at - and the red/black split means no particle is written twice inside a batch, i.e. a batch has no internal ordering at all. That is what makes a VU0 microprogram a port rather than a rewrite (docs/cloth.md, "Moving it onto VU0", which also records the 256-quadword data memory and the no-cross-element-access rule that decide what such a kernel could actually take). **THE STEP IS FIXED** at 50 Hz with a bounded catch-up: Verlet with a varying dt changes its own stiffness every frame and explodes on the first loading hitch. Two smaller things worth not re-deriving: the gust costs ONE `sinf`/`cosf` for a whole sheet (each row's phase offset is baked as a sin/cos pair at reset and combined by angle addition), and `anchor()` re-places the PINNED particles on the object's live transform before every step, which is what lets a curtain be carried by a moving object with no extra machinery. **The `w` lane is garbage by construction on the console** - `Tyra::Vec4`'s default ctor initialises nothing and its xyz operators leave w alone - so the solver never reads it (`innerProduct` and `cross` are xyz) and the runtime writes `w = 1` EXPLICITLY into the vertex array, which VU1's MVP multiply does read. Cloth is deliberately outside every static system: no collision (`placement::collides`, `objectCollides`), no placement surface, no lightmap region or occluder (it falls through `aobake`'s defaults), no static batch, no texture atlas, no navmesh, no raycast, no USE target - a surface that MOVES is the one thing this engine's bakes cannot describe. **`PrimitiveType::Wind` (22) lives here too** (docs/cloth.md, "Wind entities"): `cloth::Wind` + `windAt`, the placed source that blows every sheet in reach along its own local +Z. Two rules it exists to enforce. The wind a sheet feels is a **VECTOR** - its own draught plus every source - which is the only reason several sources compose; that is why `Params::windAccel` replaced an amplitude and a bearing, and the gust envelope is now a scalar applied to it. And the sum is sampled **once per SHEET, at its origin**, never per particle: a sheet is small against the distances a source works over, so per-particle sampling buys a gradient nobody can see for a subtract, a dot and a compare per particle per source. The falloff is `1 - d^2/r^2` - the engine's own point-light curve, not a new one - and radius 0 means the whole scene with no falloff. Only STRENGTH and REACH are baked into `WINDS`; position and direction are read off the live `RuntimeObject` every frame, which is what lets a fan be carried, and `visible` is what Hide Object switches. |
...
**New object type** → `PrimitiveType` enum (0–22 used so far, `kPrimitiveTypeCount`
