...
| `iso9660.cpp`, `isoexport.cpp`, `esrudf.cpp` | 384+270+~340 | In-tree ISO9660 writer + disc layout planning (`Project > Export PS2 ISO`, Disc Layout window). `isoexport::buildEsr`/`Project > Export ESR ISO` (+ CLI `--export-esr`) layers on `esrudf`: a minimal UDF 1.02 bridge whose partition is ESR-patched to a fake DVD-Video structure so a modded PS2 boots the DVD-R via the ESR loader (game itself still boots over ISO9660). The UDF-partition patch + fake-partition/CRC tables (`esrudf_data.inc`) are a port of [esrtool](https://github.com/ali-raheem/esrtool) (MIT). ESR mode drives `iso9660::write`'s `Options{firstDataLba=257, tailReserveSectors=1}` to reserve the UDF sectors; the plain path uses the defaults and is byte-identical to before. |
