...
| `viewport.cpp/.hpp` | 1184 | Offscreen GL 3.3 preview: unit-primitive meshes, terrain grid + heightmap, sky dome, selection outline (+ per-peer session-presence outlines via `setPeerSelections`, drawn under the local amber), live point-light shader, sculpt-brush raycast, orbit/pan camera. Also the Material Editor preview (a primitive, a project .obj, or an animated .glb/.fbx in bind pose via `buildMatPrevAnimated` - the assigned .mtl resolved name-matched like the console, with the selected entry's staged values), its paint raycast (`materialPreviewPick`) and the live painted-texture upload (`updateTexturePixels`, shared texCache_ id so the scene updates too). **Every preview window owns its own render target** (`prevFbo_/prevTex_` material, `animFbo_/animTex_` animation, one per future preview) - two tool windows are routinely open at once and size themselves independently, so a shared target both thrashes its `glTexImage2D` size and makes each `ImGui::Image` show the other's draw; only the backdrop meshes are shared (`ensurePreviewBackdrop`). Shading is BAKED into vertex colors (`shadeOf`, the `g*` light globals), so a preview lighting override (`PreviewLight`) is a re-bake of the preview's own meshes under a scoped global swap (`ScopedShade`, private unit-shape copies, the light folded into the `matPrevModel_` key) - never `setLighting`, which is the scene-wide setter and rebuilds every mesh incl. the terrain AO grid. |
