...
| `vu_ui.cpp` | ~560 | The **VU Programs** window (Tools > VU Programs, docs/vu-authoring.md) - App:: methods declared in app.hpp, own TU (the assetbrowser.cpp precedent). Composing a VU1 microprogram or a VU0 kernel out of STAGES with no assembly. What makes it more than a form over a struct is that it answers the two questions authoring a microprogram actually raises, with no Docker and no console: *does it fit* (the micro-memory bar against the real 2042-slot ceiling, with the resident mask auto-detected from what the scenes and prefabs draw - `vuNeededClasses`) and *what does it do* (`vuSimulate`: the program run in the host VU simulator over a synthetic mesh, printed as GS vertices TWICE - once with the panel's mesh parameters and once with them at zero, which is the identity contract shown rather than asserted). Both go through the same `vugen::build` the BUILD uses (`vuRebuildPreview`, rebuilt every frame the window is open - milliseconds), so the listing, the budget and the simulation cannot become three answers to one question. Per-mesh numbers live in the object inspector (props_ui.cpp) and appear only when the project has a program, each slot labelled with the stage parameter that reads it. |
...
| `flownode.cpp` | 230 | Loads project-defined **custom flow nodes** from `<project>/flow-nodes/*.flownode` text files into the global `customFlowNodes()` registry (`flownode::loadForProject`). Called by `project::load` *before* graphs are parsed. Parses the manifest (title/category/params, `in`/`out` pins, `exec_out`, `call`) into a `FlowNodeType`; the node's behavior is an inline C++ snippet or a `call = fn` into `inc/scripts/flow_nodes.hpp`. Also scaffolds the starter file (`writeExample`). See `docs/custom-flow-nodes.md`. **When you add/change a header key or `{placeholder}` here, mirror it in the VS Code extension's `SPEC` table (`tools/vscode-tyrax/extension.js`) and the grammar**. **VU1 microprograms have their own language in that extension too** (`tools/vscode-tyrax/vu.js`), but its macro/address catalogue is SCANNED from the engine tree rather than copied - so adding a `#macro` needs no extension change, while adding a VU *instruction* to `vuir` means adding its one-line description to `OPCODES` there — that is what colours/validates `.flownode` files (see `docs/vscode-extension.md`). |
...
error with the author's line. Five traps, all measured on hardware. A SCRIPT MUST NEVER WRITE Q (div, rsqrt,
sqrt): Q carries the perspective divide, it has a latency vcl schedules around,
and a Q write from a script body gets moved into that window - vertices land in
the wrong place and it shows as grey stipple plus z-fighting shadows on the
console while the host simulator, in order and latency-free, shows nothing.
vugen refuses it now.  MULTIPLY, NEVER ADD, and never touch
