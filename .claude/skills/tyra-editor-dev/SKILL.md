...
| `iso9660.cpp`, `isoexport.cpp` | 379+280 | In-tree ISO9660 writer + disc layout planning (`Project > Export PS2 ISO`, the Disc Layout window, `--export-iso`; docs/disc-image.md). The **title id** (`ProjectSettings::titleId`) is resolved here through `project::discBootFileName` - the boot file lands on the disc under the id ("SLUS_213.45"-shaped) the way a retail disc names it, while **`Project::elfName()` is untouched**: the ELF in `bin/` keeps its own name for every `host:` path there is (the Runner, ps2link, all four devkit channels), so the id renames only the copy that goes on the disc. `OrderedFile::isoName` is the per-file override that carries it. The id also derives the memory card folder (`project::saveGameDirName`, baked as `SAVE_MC_DIR`), which is why both live in project.cpp as `(titleId, name)` overloads as well as `(Project)` ones - the preferences dialog previews an id it has NOT applied yet, and a preview that read the saved project would answer for the id being replaced. An **empty** title id means both pre-title-id behaviors (the ELF's own boot name, the `TYRA-<NAME>` save folder), which is what a project saved before the key loads as - never make it fall back to a generated id, or an existing game's saves move behind its author's back. |
