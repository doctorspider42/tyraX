...
| `uvunwrap.cpp/.hpp` | ~300 | **Automatic UV unwrap** for static .obj models (Material Editor > UV check > "Unwrap UVs..."). Smart-project: normal-clustered charts (angle threshold, BFS over shared edges), per-chart planar projection + tightest-bbox rotation, ONE global scale (uniform texel density), shelf packing with a bleed margin. Rewrites the .obj IN PLACE preserving every non-vt line byte-for-byte (face `v`/`vn` parts untouched, new vt pool inserted before the first face). Deterministic. The UI invalidates viewport/model/bake caches and auto-runs the validator after. |
