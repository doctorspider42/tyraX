...
### Round six: retained uniform blocks, the spot cache and two dead uploads - PCSX2 ONLY, 2026-09-26
...
directions and colours, or an unlit bag's three object-space spot quads and
the custom VU params) and the MATERIAL group (single colour,
OPTIONS/TEX1/TEST/TEX0, billboard and env bases, ALPHA). Each group is built
by the unchanged builder into a hot stack scratch and compared with the copy
the bag's entry retains, stored as a plain VIF stream like the round-five clip
block. Equal: the packet carries ONE REF (4-16 quadwords fewer written into
cold packet lines, and nothing new to write back). Different: the copy is
rewritten and REF'd if no chain can still read it, otherwise the group goes
inline exactly as before. The MVP stays inline, because it moves with the
camera.

- **The key is the output.** The backlog asked for a key over the inputs (MVP,
  light, spot, fog facts, material, z-test, texture VRAM address) and warned
  that the risk is an input the key cannot see. This design has no such key: a
  hit is a byte compare of the freshly built group against the retained copy.
  A re-shade, a camera-dependent fog decision, or a texture evicted and
  re-uploaded at a new address all change the built bytes, and a changed byte
  is a miss. Which entry a bag uses (a hash of its vertex, info, texture and
  colour pointers, 256 sets x 2 ways) is only a placement hint. Two colliding
  bags take turns and cannot draw each other's uniforms.
- **The lifetime rule** is the packet-buffer rotation's. Each group remembers
  the renderer's packet serial that last REF'd it. `sendPacket` records which
  `Vif1Queue` sequence each serial became. A copy is rewritten only when its
  packet was submitted and `Vif1Queue::isComplete(seq)` says so. That is a new
  READ-ONLY query of the completion counter: it never advances the queue (the
...
- **The saving is bounded by what it removes.** Per bag the EE still builds
  the group (same loads, same arithmetic) and now also reads the retained copy
  to compare. What goes is the stores into cold packet lines and their
  write-back at the next `FlushCache`. Round three priced 16 cold qwords at
...
over the spot arithmetic on OPTIONS.y. `LoadTyraSpotLight` still loads the
three quads, but only `CalculateTyraSpotLight` uses them, and that runs only
when the lane is negative. So an inactive spot uploaded dead data. The
quadwords are still sent whenever a game program override is installed,
because that code may read them unconditionally.

**4. The clip block only for a bag that can clip
(`TYRA_STAPIP_CLIP_BLOCK_GATE`).** In VU1-clipping mode every bag REF'd the
15-qword clip block, but only the `clip_*` programs read
`VU1_CLIP_CONSTS_ADDR` and the plane table. The one route that reaches a
clip program is PARTIALLY_IN_FRUSTUM with `fullClipChecks`. StaPipCore knows
that before `sendObjectData` (`setBagMayClip`, after the guard-band
promotion), so whole-IN, guard-band-direct and cull-all bags leave it out.
Each clipping bag still uploads the block itself, so another pipeline reusing
that VU1 memory in between cannot matter.

**Measured in PCSX2** (counts and pictures only). The district fixture was
regenerated by the branch's editor. Arms were built with
`C:\tyra-vq\make_bag_engines.py` + `build-bag-arms.ps1`:

| check | result |
| --- | --- |
| VIF errors (`smoke.ps1`), every arm | none |
| `TYRA_STAPIP_UNIFORMS_VERIFY` (packet = old bytes, retained path shadowed, every would-be REF compared with the old bytes as a VIF stream, every spot-cache hit compared with a fresh `buildSpotForBag`), all four poses + held pose | **0 mismatched** of ~22 000-58 000 checks per 300 frames |
| VIF-hash gate, one ELF, mode 0 vs mode 6 (spot cache + retained uniforms), garage day | `ctrl`, `uni`, `pool` identical in **24 / 24** frames; `chainQw` smaller (the REFs), `geo` differs in both arms as it always has |
| retained groups (`TYRA_STAPIP_UNIFORMS_REPORT`, 300-frame windows) | 34-100 REF hits a frame over the benchmark phases, 38 in the held outer pose; 0.3-4 rewrites a frame (most in the night windows), 0.7 held; inline 0 (0.8 a frame in one verify boot: two-way set collisions at night) |
| captures (`pic-bag-pcsx2.ps1`, HUD rows masked; old/new builds + one ELF booted mode 0, 1, 0, 1) | garage day **byte-identical in all 16 captures**. Outer day: every capture is one of two images, and both occur in old-build boots and in mode-0 AND mode-1 boots (one mode-1 boot shows both), so that is boot state, not this change. Night: differences only in the flickering lamp areas, as large within one arm (up to ~1600 px) as between arms |

The VIF-hash gate cannot cover modes 8 and 16: they change what is uploaded,
not how it is chained. The gate itself latches `BROKEN cmd=0x80` after the
garage-day window in BOTH arms (mode 0 too), so its hashes only speak for
garage day. That is a limitation of the gate on this fixture and is not
...
- **The lifetime argument on hardware.** PCSX2 finishes a DMA before the EE
  ...
  packet-buffer rotation waits on. A console failure-rate check at night (the
  pose with the most rewrites) is owed before this ships: 24-30 captures
  scored against a reference, as in the slot-pool race.
- **The parked fixture flatters it.** Every pose holds the camera. With a
  moving camera the MVP changes every frame (it is inline anyway), and so does
  the object-space spot of a camera-attached flashlight, so at night the light
  group rewrites every frame while the material group still hits. A
  `--keep-routes` fixture under the verify arm is the adversarial check for
  moving traffic.
- **Microcode left alone, on purpose** (another change owns it). Untextured
  C/D programs still emit a TEX1 in their GS state (2 qwords a package), and
  `cull_c` ignores `VU1_STAPIP_EMIT_STATE_FLAG`, so it resends its 7 qwords of
  GS state for every package. The EE side already sets that flag correctly
  for every program, retained replays included. The microcode side needs to
  drop TEX1 from the C/D GIF tag's register list and branch over the state
  block when the flag is clear, as the programs that honour it do. The EE keeps
  sending TEX1 in OPTIONS, which then goes unread and is harmless.
