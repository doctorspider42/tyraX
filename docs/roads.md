...
- **Intersection texture** enables automatic junctions. When two roads in the
  same scene cross and both name the same non-empty image, codegen samples the
  ...
  order. Crossings below roughly 14 degrees are also left alone: their overlap
  is an unbounded needle and should be authored as a merge. The viewport shows
  the same generated patch and texture.
