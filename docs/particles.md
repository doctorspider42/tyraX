...
Every kind reaches exactly zero before the quad's edge. The texture is pinned
to **full colour** in `textureQuality` (a soft alpha ramp does not survive the
...
an emitter's `"effect"` and `"additive"`, a vehicle's `"smokeEffect"`. All are
written only when set, so a project that uses none resaves byte for byte.

## Limits

- One texture per emitter (no flipbook animation yet).
