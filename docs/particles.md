...
*Tools > Vehicle Editor > Effects > Tyre smoke* picks the look of a
definition's smoke from one list: the built-in puff, a **particle-library
effect** (`VehicleDef::smokeEffect`) or a plain material
(`VehicleDef::smokeMaterial`, see [vehicles.md](vehicles.md), "Skid marks and
smoke"). Picking one clears the other; a file that names both (hand-edited)
uses the effect. WHEN and WHERE puffs spawn stays the sim's - the tyre slip
decides, the rear wheels place them, drag and buoyancy move them (see
[vehicles.md](vehicles.md), "Tyre smoke"). The effect supplies the look
(`vehbake::smokeLookOf`, the one mapping):

| effect field | smoke |
|---|---|
| tint | puff colour (x128 over a texture, x255 untextured) |
| opacity | peak alpha |
| size, grow (Custom) | start size (0.6 x size), end size (other motions grow 2.4x) |
| lifetime | life multiplier (1.5 s = the built-in) |
| gravity < 0 (Custom) | rise multiplier (0.3..3) |
| texture | the puff's billboard, flipbook frames included |
| additive | GS additive blend (FIX 128), the fade carried in the colour |

Codegen bakes one `VEHICLE_SMOKE_LOOKS` row per definition (in
`inc/scene_data.hpp`, next to `VEHICLES`): `effect` 1 plus the numbers and the
frame materials, or an `effect` 0 row for a definition that keeps its material
or the built-in puff. **Every definition is its own pool** (the vehicles
runtime's `VehFx`), so each car's effect keeps its own texture and blend - the
pool is still one bag and **one submit per definition**, VU1 expanding the
billboards, and an empty pool is not submitted. A flipbook swaps the bag's
texture pointer at the effect's fps, nothing else. The textures come through
the refcounted texture cache (`acquireTexture`), like a definition's own
smoke material, so they do not join `MATERIAL_PATHS` and need no residency
entry. Only the effect's main layer is used.

**The built-in puff is this library's smoke texture.** `vehicles/fx-smoke.png`
is `particletex::generate` of a fixed recipe (`vehbake::builtinSmokeRecipe`:
kind 1, 64 px, seed 9, softness 0.85, detail 0.4, scale 0.9) with its alpha
scaled by 0.91 to keep the old hand-written puff's coverage - one procedural
smoke generator in the editor, not two. **New library smoke** under the combo
adds a "Tyre smoke" effect with that recipe and the built-in sizes, life and
opacity (`vehbake::tyreSmokeEffect`), links the car to it and opens the
Particle Editor on it, so tuning a car's smoke starts from what it already
looks like.


Format v68 (shipped on the particle branch as v62; renumbered when it merged
into the vehicles branch, which had taken v62..v67): the `"particleEffects"` section (`project::Section::Particles`,
...
- The vehicle backfire flash is still an untextured quad in the shared glow
  bag, not a library glow (docs/backlog.md).
