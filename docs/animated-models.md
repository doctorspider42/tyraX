...
| The editor pauses the first time a model is assigned | Expected, and it should be brief. Picking a model bakes every clip into morph frames once (the viewport preview and the panel summary each keep their own cache, so the file is read twice), and the cost scales with clips x duration x vertices: a 43-clip 4.7k-vertex character is ~0.4 s per bake. Seconds rather than fractions mean the file took the slow posing path - an .fbx whose nodes use parent-scale inherit modes, or whose skin blends dual-quaternion, is posed by ufbx's own evaluator, which copies the whole scene per baked frame (~15x). |
