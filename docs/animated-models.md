...
A walk clip is authored on a flat floor, so on stairs, a curb or a slope the
same clip puts one shoe inside a step and the other in the air. **Foot IK**
post-processes the pose after animation: it raycasts the ground under each
foot, plants the one taking weight, solves the leg to reach it and lowers the
pelvis by whatever the leg itself cannot cover.

The rig is authored ONCE PER MODEL in **Tools > Foot IK** — which bones are
legs, the tolerances, per-clip rules — and each instance carries only a
**Foot IK** checkbox, so the same character can have it on for the player and
off for a background NPC. *Preferences > Build > Show Foot IK probes* draws the
solver's own raycasts in a debug build, which is how a foot left in the air
tells you whether the ray missed, a gate refused the surface, or the solver
lifted the shoe on purpose.

Full guide: **[foot-ik.md](foot-ik.md)**. The example project is
