...
**Measured on the physical PS2 (1.145.1).** Medians of `work` over two
boots per mode, one ELF per fixture. Each figure is what turning the knob
OFF costs against the shipped runtime (mode 0):

| lever | district fixture (4 poses) | multi-car orbit (4 phases) |
| --- | --- | --- |
| vehicle lights keep (54) | +0.27..+0.30 ms, day and night | +0.32..+0.47 ms |
| paint one-a-frame (53) | - | +0.11..+0.27 ms |
| paint distance step (52) | - | +0.04..+0.14 ms |
| beams keep (55) | +0.02..+0.03 ms | - |
| everything off (59) | +0.27..+0.29 ms | +0.36..+0.65 ms |

The second sub-step's gather reuse, with two sub-steps forced every frame:
+0.27..+0.32 ms with the reuse (mode 43) against +0.50..+0.52 without it
(mode 56). So a 25 FPS frame's second vehicle step went from ~0.5 to
~0.3 ms.

The headlight pools were the big one. A parked car rebuilt its two pools (16
ground queries and 54 vertices each) and re-staged them every frame, by day
too, because a car's lights are "on" whether or not the scene is dark. The
beams barely matter: most of their cost is fill, not EE.
