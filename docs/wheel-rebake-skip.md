...
### Nothing is added to submission, and three counters say so independently

This is worth stating plainly because a reasonable person reading "fixed-size
per-car spans" will expect the spans to be padded with degenerate triangles.
**They are not.** The buffer is resized to exactly `cars * vertsPerCar` at the
end of every frame, which is the same length the old clear-and-refill produced,
so the bag's `count` is identical car for car. Measured on two arms whose
fixtures differ in exactly one generated file:

| pose | triangles, both arms | flushes, both arms | uploads | re-uploads |
| --- | ---: | ---: | ---: | ---: |
| garage day | 40 502 | 120 | 0.000 | 0.000 |
| garage night | 41 176 | 142.5 | 0.000 | 0.000 |
| outer day | 16 386 | 44 | 0.000 | 0.000 |
| outer night | 16 720 | 48 | 0.000 | 0.000 |

Exactly equal, not approximately. The retained-command totals agree
independently — `retained + rebuilt` per frame is 869, 909, 590 and 377 in
**both** arms — and the pixel hashes above are the third line of the same
evidence: identical bytes out of identical geometry.

**If a comparison shows the triangle count moving, the arms differ in the
fixture and not in this change.** 40 502 is the count this scene reports when it
is *regenerated by the editor under test*; `benchmark-district.py` copies the
example's **committed** generated sources, which drift, and a stale fixture
reports a different count and about 3.6 ms more render submission (its own
docstring, and docs/gs-vram.md). The cheap discriminator is the `reuploads`
column: a regenerated fixture reads 0.000 in all four poses.
