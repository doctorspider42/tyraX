...
## VU1 audit: what is left (2026-09-26)

Shipped on `vu1-audit-fixes`: resident billboards, clip TD on the TC image and
three output-preserving trims (docs/vu1-clipping.md, "The resident set after the
VU1 audit"). `--vu-check` green and PCSX2 pixel-identical at the four district
benchmark poses; **the physical-PS2 pass is owed** - the same four poses,
baseline vs this branch, `district-benchmark.csv` medians plus one
`--profile-frame` per pose. What to read: `Program_swaps_count` 0 and
`Program_swap_wait_included` 0.000 on the branch (non-zero on the baseline
wherever smoke/exhaust is in view), `VU1_wait_included` and `Total` per pose.

Not built, because each needs cycles measured on the console rather than a
simulator verdict. Each belongs behind a compile-time toggle, one arm per toggle:

- **Constant 255 and a table ADC bit in `cull_tc`/`cull_c`.** Keep 255 in a VF
  register instead of `loi 255` before every `mini` (FixColor, CalculateTyraFog)
  - that frees a lower-pipe slot per clamp at the price of one VF register
  (`cull_tc` estimates 29 of 31). Replace `iaddiu adcBit, VI01, 0x7FFF; iand
  adcBit, adcBit, adcMask` in PerformTyraFogClipCheck with `ilw.x adcBit,
  <table>(VI01)` over a two-entry table {0, 0x8000} at 1019..1020 (1016..1018
  hold the single-colour copies now) - bit-identical by construction (fcand
  yields 0/1), frees `adcMask`, but trades two integer ops for a load whose
  latency may not hide. NOT the "only vertex 3's flags" shortcut: strips and the
  0x3FFFF window make vertices 1-2 matter. Console: build the fixture twice from
  `examples/vehicle-playground/authoring/benchmark-district.py` with the toggle
  0/1, boot each over ps2link, read the four medians; decide on garage NIGHT
  (the lit loop) and outer-road DAY (the unlit loop), the two paths that matter.
- **Light-matrix multiply folded on the EE (TD/D).** Upload the three light
  directions pre-rotated by the model's normal matrix, so
  `CalculateTyraDirectionalLights` drops its first three instructions per
  vertex. Not bit-identical (EE rounding differs from VU1's), so the gate is a
  PCSX2 pixel A/B within 1 LSB plus the console medians; the district's lit
  meshes are few, so pick a pose where `cull_td` actually runs (check with the
  packet tap first - "AIM A VU1 EXPERIMENT AT THE PROGRAM THE SCENE ACTUALLY
  RUNS" in the engine skill).
- **`cull_tce` renormalize.** Normalize the captured env normals once on the EE
  (`src/templates.cpp` ~20490-20522) and drop the rsqrt chain from `cull_tce`
  and the TC image's env path (their ST is computed on the original corners);
  `as_is_tce` keeps its normalize, because the EE clipper interpolates normals.
  Not bit-identical; the vehicle paint is the consumer, so A/B a parked car
  (`make_mc_arm.py`) in PCSX2 for pixels and on the console for `cull_tce` time.
