...
benchmark poses. **Physical PS2, 1.146.0** (baseline vs branch, A B A B, each
arm generated by its own editor, medians): `work` -0.50 / -0.44..-0.46 / -0.38
/ -0.41 ms at garage day / garage night / outer day / outer night, the two
branch boots agreeing to 0.02. The split: `vif_wait` -0.13..-0.22 (VU1 does
less per vertex) and `dispatch` -0.18..-0.25.
