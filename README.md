...
- **Material Editor & texture painting** (*Tools > Material Editor*) — authors the `.mtl` materials the whole pipeline consumes (color × brightness + `map_Kd` texture, saved on every change, scene viewport updates live). The preview shows a unit primitive **or any of your `.obj` models** (the open file acts as the model's material override, exactly like in the game; drag orbits, wheel zooms). **Duplicate** copies a material together with its referenced textures (repaint-safe). **Paint** draws straight onto the preview mesh through its UVs — a soft color brush or a **texture brush** that tiles a pattern PNG continuously across the surface — with per-stroke undo; every stroke saves the flat PNG, which is exactly what the PS2 loads (no separate bake). *Texture > New paintable texture...* creates a blank pow2 canvas next to the `.mtl` for texturing a model from scratch. Full guide: [docs/material-painting.md](docs/material-painting.md).
