...
- **Flow graph** — visual logic (CryEngine-style): wire triggers (On Start, On Button, Near Object, Every N Seconds) to actions (Set Sky Color, Show/Hide/Toggle/Move/Recolor Object, Play/Stop Sequence, Log) in the *Flow Graph* tab; the graph compiles into `src/scripts/flow_graph.gen.cpp` on every build.
- **Cutscene Director** (*Tools > Cutscene Director*) — a keyframe **timeline sequencer** for cinematic cutscenes on the PS2, with a dopesheet UI: one lane per animated object plus a camera lane, keyframes as draggable diamonds (click to select, drag to retime, right-click for easing/delete, double-click a lane to drop a key), a click-scrubbed time ruler with zoom, and a playhead that previews the whole scene live in the viewport. A *sequence* poses scene objects (position/rotation/scale/color/visibility) **and the game camera** over time with per-key easing (linear / smooth / step). Camera keys are *shots*: free (snapshot of the editor view, explicit eye/look-at/FOV) or **bound to a Camera entity** (`+ Add object > Gameplay > Camera` — a film-camera marker with an FOV frustum wedge in the viewport); a bound shot films from the entity's live pose, so keyframing the entity itself in an object track gives dolly/crane moves, and Step easing between two shots is a hard cut. Shot FOV drives the **real PS2 projection** (restored when the cutscene ends), per-shot **camera shake** adds handheld energy, and each sequence can be **skippable (START)**, **fade in/out from black** and mask the frame with **widescreen bars** (Cinema 2.39:1, Wide 16:9, Pillarbox, Frame) that slide in and composite over the HUD. Fire it from the flow graph with the **Play Sequence** / **Stop Sequence** nodes (e.g. On Used → play a cutscene); the camera track takes over the game camera for the duration and hands it back when the sequence ends. Compiles into `src/scripts/sequences.gen.cpp` (a runtime player) on every build.
- **Phone camera takes** (*Cutscene Director > Import take...*) — record a real 6DoF camera move on a phone (ARKit world tracking, e.g. the [CamTrackAR](https://fxhome.com/product/camtrackar) iPhone app) and import it as free camera shots on the sequence's camera lane: "walk around a room looking around" becomes a PS2 cutscene camera move. Reads CamTrackAR `.hfcs` exports and an app-agnostic CSV (spec in [docs/camera-takes.md](docs/camera-takes.md)); the import modal maps the take into the scene (scale "1 m = N units", extra yaw, origin placement at the preview camera, start-at-playhead) and **decimates** the 60 Hz recording with an error-bounded tolerance slider (a 6.6 s take → ~12 keys) so the compiled PS2 keyframe table stays small. Imported shots are ordinary editable keys — scrub, retime or delete them like hand-authored ones. The acquisition/bake split is ready for phase 2: live pose streaming from the phone into the same pipeline.
