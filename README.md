...
- **Flow graph** — visual logic (CryEngine-style): wire triggers (On Start, On Button, Near Object, Every N Seconds) to actions (Set Sky Color, Show/Hide/Toggle/Move/Recolor Object, Log) in the *Flow Graph* tab; the graph compiles into `src/scripts/flow_graph.gen.cpp` on every build. Need an action the editor doesn't have? Define your own **custom node** in a `flow-nodes/*.flownode` text file (a tiny header + a C++ snippet) — no editor rebuild — and copy the file to reuse it in other projects: [docs/custom-flow-nodes.md](docs/custom-flow-nodes.md).
