...
- **Usable-object highlight** — objects marked *Usable* glow while the player is within a proximity distance (color / rim width / blur steps in *Preferences > Usable objects*, per-scene overridable). Drawn cheaply on VU1 as grown silhouette shells; an experimental **Draw over object** toggle paints the glow *on* the surface instead of only a rim behind it.
