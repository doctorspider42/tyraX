...
- [examples/reflections](examples/reflections) — [reflective materials](docs/reflective-materials.md) in both flavors side by side: chrome spheres with a **static sphere map** vs the **dynamic `@sky` mode** (the sky dome re-rendered into the map every frame, GT3-style), a car-paint variant, per-face box reflections and a matte control (see its [README](examples/reflections/README.md)).
