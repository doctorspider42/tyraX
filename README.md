...
- **Cutscene Director** (*Tools > Cutscene Director*) — a keyframe **timeline** for cinematic cutscenes on the PS2. A *sequence* poses scene objects (position/rotation/scale/color/visibility) **and the game camera** (fly-through eye + look-at + FOV) over time, with per-key easing (linear / smooth / step). Author it by scrubbing the playhead and snapshotting an object's pose or the current view; the whole scene previews live in the viewport as you scrub. Fire it from the flow graph with the **Play Sequence** / **Stop Sequence** nodes (e.g. On Used → play a cutscene); the camera track takes over the game camera for the duration and hands it back when the sequence ends. Compiles into `src/scripts/sequences.gen.cpp` (a runtime player) on every build.
