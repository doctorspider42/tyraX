...
- **Models** — `.obj` compiled into a binary
  ...
  Mixamo download onto a character you already have. Optional
  **[Foot IK](docs/foot-ik.md)** binds a character's leg bones once, per model,
  and switches on per instance: its feet land ON stairs and curbs instead of
  inside them, walking back DOWN gets the extra reach a step needs, and a
  debug-profile overlay draws the raycasts the solver actually cast.
...
| [foot-ik-stairs](examples/foot-ik-stairs) | A curb, a 22 cm flight and a 10 cm flight — one flat walk clip, and the shoes land on the treads. Untick one box to watch it without |
