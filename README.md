...
- **Live Link** — edit the **running** game: with the *debug* build profile, moving / rotating / scaling / recoloring objects streams into the game live (a gizmo drag moves the object on screen as you drag), in PCSX2 **and on a real PS2** — the snapshots travel over the same host filesystem the game loads assets from, so no extra transport exists. A toolbar indicator shows **● LIVE** while streaming; structural edits (add/delete objects, model/material swaps, point lights, projected decals) flip it to **● LIVE (rebuild)** and pause streaming until the next build — indices are never mis-patched. Master switch in *Build > Live Link*; release builds carry zero Live Link code. See [docs/live-link.md](docs/live-link.md).
