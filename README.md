...
- **Physics** — rigid-body objects (*Physics* checkbox + per-object Mass/Bounciness/Friction/Tumble): full 3D velocity, restitution bounces off terrain slopes (real heightfield normals), friction slides, tumbling, body-vs-body momentum exchange, resting on / bouncing off static solids, and player shoves. Near-rest bodies sleep, so a settled scene costs nothing; the vector math runs on VU0 (`Tyra::Vec4` macro-mode ops). Scripts get `velocityX/Y/Z` + `spin`, the flow graph gets an *Apply Impulse* node. Plus FPP player physics: gravity, jumping on X, collision with scene objects, walking on top of them.
...
- [examples/physics-playground](examples/physics-playground) — the rigid-body object physics: bodies with different Mass/Bounciness/Friction dropped on a slope (superball vs dead thud), a sleeping crate stack the player can topple, walk-into shoves, and an **Apply Impulse** flow graph that kicks a ball on a timer while logging its position (see its [README](examples/physics-playground/README.md)).
