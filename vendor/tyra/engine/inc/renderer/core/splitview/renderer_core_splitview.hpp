 ...
 * clamping with SCISSOR. The z-buffer is shared; begin() clears the half's
 * color + depth region through the scissor.
 *
 * Usage per frame (after beginFrame(cam1), which set the P1 view):
 *   core.splitView.begin(0, bgColor);       // raster -> top half
 *   ... submit the scene ...
 *   core.renderer3D.update(cam2Info);       // swap view + frustum to P2
 *   core.splitView.begin(1, bgColor);       // drains P1's half, raster -> bottom
...
class RendererCoreSplitView {
 public:
  ...
  /**
   * Drain PATH1, shift XYOFFSET/SCISSOR to the given half (0 = top,
   * 1 = bottom) and clear the half's color + depth.
   */
  void begin(const int& half, const Color& clearColor);
