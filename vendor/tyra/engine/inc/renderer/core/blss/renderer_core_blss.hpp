...
class RendererCoreBlss {
 ...
 private:
  ...
  // Modified by TyraX: the history-is-the-render-target warning has been said
  // (see composite()). It is a property of the buffering configuration, so it
  // would otherwise print every frame.
  bool histAliasWarned = false;
