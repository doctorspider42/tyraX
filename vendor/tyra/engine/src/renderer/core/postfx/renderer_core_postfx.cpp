...
RendererCorePostFx::RendererCorePostFx() {
  ...
  curFbVram = 0;
  curFbBufW = 0;
  // Sized for every pass at once: DoF (8 blits) + god rays (8) + bloom (6) +
  // grading (6 quads) + grain (2) + setup/teardown still leave headroom.
...
void RendererCorePostFx::applyCustom(CustomFxBuild build, void* user) {
  ...
  // blit()/flatQuad() (or raw PACK_GIFTAG) and advances the cursor. The 512-
  // qword packet leaves ~490 qwords after this setup: roughly 40 blits or 70
