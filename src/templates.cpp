...
std::string flowGraphScript(const Project& p) {
    ...
    for (size_t ownerIdx = 0; ownerIdx < sceneObjs.size(); ++ownerIdx) {
        ...
        // Which object a node refers to, as a C++ int-expression indexing
        // ctx.objects[...]: incoming object link (follow the chain) > a custom
        // node's runtime object output > explicit name > the owning object
        // ("self"). Built-in sources fold to a literal index at codegen; a
        // custom node with an object output instead resolves to its runtime
        // latch var (objOut<id>), so a value it computed at runtime (a raycast
        // / pick result) can drive any downstream node - built-in included.
        // "" = no valid target (an unknown object name).
        auto resolveTarget = [&](const FlowNode& n) -> std::string {
            const FlowNode* cur = &n;
            bool followed = false;  // did we arrive here via an object link?
            ...
            for (;;) {
                ...
                if (!t) break;
                // A custom node's object output is authoritative and unrelated
                // to its own inputs - stop here once we followed a link to it.
                if (followed && t->idOut && flowCustomNode(cur->type)) break;
                if (!t->idIn) break;
            ...
            if (followed && ct && ct->idOut && flowCustomNode(cur->type))
                return "objOut" + std::to_string(cur->id);
            if (ct && ct->strKind == FlowParamKind::ObjectName && !cur->str.empty()) {
                const int idx = objectIndex(cur->str);
                return idx < 0 ? std::string() : std::to_string(idx);
            }
            return std::to_string((int)ownerIdx);  // self
        };
        // A resolved target is a literal index (validated at codegen) unless it
        // is a custom node's runtime object output (objOut<id>), which may be
        // -1 (nothing) or otherwise out of range - those uses need a bounds
        // guard before indexing ctx.objects[...].
        auto isRuntimeIdx = [](const std::string& s) {
        ...
        std::function<std::array<std::string, 3>(const FlowNode&, std::vector<int>&)>
            ...
            auto objectPos = [&](const std::string& idx) -> std::array<std::string, 3> {
                const std::string base = "ctx.objects[" + idx + "].data.position[";
            ...
            const std::string idx = resolveTarget(n);
            if (idx.empty()) return {"0.0F", "0.0F", "0.0F"};
        ...
        auto sourceCondition = [&](const FlowNode& n) -> std::string {
            ...
            if (n.type == "IsVisible") {
                ...
                if (isRuntimeIdx(idx))  // guard a possibly-invalid runtime ref
                    return "((" + idx + ") >= 0 && (" + idx +
                           ") < ctx.objectCount && ctx.objects[" + idx +
                           "].visible)";
                return "(ctx.objects[" + idx + "].visible)";
            }
        ...
        auto actionCode = [&](const FlowNode& n, const std::string& pad) -> std::string {
            ...
            if (n.type == "SetSky") {
                ...
                if (!cn->callFn.empty()) {
                    ...
                    c << p2 << "io.object = " << (idx.empty() ? "-1" : idx) << ";\n";
                    c << p2 << "io.num = num;\n";
                    ...
                    body = replaceAll(body, "{obj}",
                                      idx.empty() ? std::to_string((int)ownerIdx) : idx);
            ...
            // A built-in object action fed a runtime object ref (a custom
            // node's objOut, possibly -1) is bounds-guarded so an invalid pick
            // is a no-op instead of an out-of-range ctx.objects[] access.
            // Custom nodes get the raw index in io.object and check it in C++.
            if (isRuntimeIdx(idx) && !flowCustomNode(n.type)) {
                const std::string inner = c.str();
                std::ostringstream g;
                g << pad << "if ((" << idx << ") >= 0 && (" << idx
                  << ") < ctx.objectCount) {\n";
        ...
        for (const FlowNode& n : fg.nodes) {
            if (n.type == "Delay") {
                ...
                const std::string obj = "ctx.objects[" + idx + "]";
                // Guard a runtime object ref (custom objOut, maybe -1/invalid).
                const std::string guard =
                    isRuntimeIdx(idx) ? " && (" + idx + ") >= 0 && (" + idx +
                                            ") < ctx.objectCount"
                                      : "";
                clsOut << "    if (" << var << guard << ") {\n"
