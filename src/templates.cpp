...
std::string sequencesScript(const Project& p) {
    ...
    out << "// Generated by tyra-editor from the Cutscene Director. Do not edit -\n"
        ...
        << " {\n"
           ...
           "struct CamKey { float t; float eye[3]; float at[3]; float fov; int ease; };\n"
    ...
    for (size_t si = 0; si < p.sequences.size(); ++si) {
        ...
        out << "static const CamKey " << sp << "Cam[] = {";
        {
            ...
            for (size_t ci = 0; ci < ck.size(); ++ci) {
                ...
                out << (ci ? ", " : "") << "{" << floatLit(k.time) << ", " << v3(k.eye)
                    << ", " << v3(k.target) << ", " << floatLit(k.fov) << ", " << k.easing
                    << "}";
            }
            if (ck.empty()) out << "{0.0F, {0,0,0}, {0,0,0}, 60.0F, 0}";
        }
        out << "};\n\n";
    }
    ...
    for (size_t si = 0; si < p.sequences.size(); ++si) {
        ...
        out << (si ? ", " : "") << "\n  {\"" << escapeCString(s.name) << "\", "
            ...
            << (s.cameraEnabled ? 1 : 0) << ", " << sp << "Tracks, " << s.tracks.size()
            << ", " << sp << "Cam, " << s.cameraKeys.size() << "}";
    }
    if (p.sequences.empty())
        out << "{\"\", 0.0F, 0, 0, nullptr, 0, nullptr, 0}";  // non-empty array
    out << "\n};\n"
...
// The Cutscene Director: one global Script. While a sequence is active it poses
// every track's object (in the matching scene) and, if the sequence has a
// camera track, requests the frame camera override the game applies before
// rendering. Playback advances by the real frame dt so cutscenes run at a fixed
...
class SequenceDirector : public Script {
 ...
 public:
  ...
  void update(ScriptContext& ctx) override {
    if (active_ < 0 || active_ >= kSeqCount) return;
    ...
    if (s.camEnabled && s.camKeyCount > 0) {
      ...
      const float t = time_;
      auto cam = [&](int comp, int which) -> float {  // which: 0 eye, 1 at
        auto val = [&](int i) { return which == 0 ? k[i].eye[comp] : k[i].at[comp]; };
        if (t <= k[0].t) return val(0);
        if (t >= k[n - 1].t) return val(n - 1);
        int i = 0;
        while (i < n - 1 && t >= k[i + 1].t) ++i;
      ...
      ctx.cameraEye.x = cam(0, 0);
      ctx.cameraEye.y = cam(1, 0);
      ctx.cameraEye.z = cam(2, 0);
      ctx.cameraAt.x = cam(0, 1);
      ctx.cameraAt.y = cam(1, 1);
      ctx.cameraAt.z = cam(2, 1);
    }
    time_ += g_frameDt;
    if (time_ >= s.duration) {
      if (s.loop) {
