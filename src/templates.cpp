...
// src/scripts/live_link.gen.cpp - the Live Link poller. Debug builds only:
// the editor mirrors scene edits (object transforms/colors) into the running
// game by writing bin/livelink.bin next to the ELF, which the game already
// reaches over host: (PCSX2 Host Filesystem or the ps2link file server - the
// same mechanism on both, no extra transport). This generated global Script
// polls that file every few frames and patches the runtime objects; a release
// build compiles to an empty translation unit. The editor only writes
// snapshots while the built structure signature (bin/livelink.sig, written by
// the Runner at build start) still matches the project, so object indices
// here are always the authored scene_data order. Binary layout (little-endian,
// both sides are LE):
//   u32 magic 'TXLL', u32 version, u32 seq, i32 scene, i32 count, u32 reserved
//   count * 12 floats (position3, rotation3 deg, scale3, color3 0..1)
...
static std::string liveLinkScript(const Project& p) {
    ...
    if (p.settings.buildProfile != "debug") {
        out << "// Live Link is compiled only into debug builds (Project > "
               "Preferences >\n// Build profile \"debug\"); this release build "
               "carries no poller.\n";
        return out.str();
    }
    ...
    out << "// Live Link (debug builds): applies editor edits to the running "
           ...
           "// Largest authored object table across scenes - the read buffer "
           "bound.\n"
           ...
           "\n"
           "    static unsigned char buf[LL_HEADER + LL_MAX_OBJECTS * LL_STRIDE "
           "+ 4];\n"
           ...
           "    if (scene != ctx.scene || count < 0 || count > LL_MAX_OBJECTS) "
           "return;\n"
           ...
           "    int n = count;\n"
           "    if (n > SCENE_OBJECT_COUNTS[ctx.scene]) n = "
           ...
           "      // dirty only on a real change - it costs a geometry "
           "rebuild.\n"
           "      bool changed = false;\n"
