        ...
        if (ImGui::BeginCombo("Model", current.c_str())) {
            ...
            for (const std::string& m : anim) {
                ...
                if (ImGui::Selectable((m + " (animated)").c_str(),
                    ...
                    o.footIk = false;  // the new model has its own rig, if any
                    committed = true;
                }
            }
...
bool App::drawFootIkRow(SceneObject& o) {
    if (!isAnimatedModelPath(o.modelPath)) return false;

    bool changed = false;
    ImGui::SeparatorText("Foot placement");
    changed |= ImGui::Checkbox("Foot IK", &o.footIk);
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip(
            "Run the ground solver on THIS character: post-process the sampled\n"
            "clip so both ankles stop on terrain and solid geometry.\n\n"
            "Which bones are legs, and how far a shoe may reach, belong to the\n"
            "MODEL - bind them once in Tools > Foot IK and every instance\n"
            "shares the binding.");
    ImGui::SameLine();
    if (ImGui::SmallButton("Foot IK tool...")) {
        footIkModel_ = o.modelPath;
        showFootIk_ = true;
        pendingFocusWindow_ = "Foot IK";
    }
    // An instance switched on with no bound rig would silently animate as
    // authored, which reads as "the checkbox does nothing" - say why instead.
    if (o.footIk && !footIkRigBound(o.modelPath))
        ImGui::TextColored(ImVec4(1.0f, 0.7f, 0.3f, 1.0f),
                           "This model has no Foot IK rig yet - bind its leg "
                           "bones in Tools > Foot IK.");
    return changed;
}
