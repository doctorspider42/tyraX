...
// One-line semantics per built-in node key. The pin layout and param labels
// come from the registry; this adds only what the registry cannot express.
// A node missing here still gets its pins/params listed - keep entries for
// anything whose behavior is not obvious from its title.
static const char* nodeDoc(const std::string& key) {
    static const std::map<std::string, const char*> docs = {
        {"OnStart", "Fires once when the scene starts."},
        {"OnButton", "Fires the frame the pad button (str) is pressed."},
        {"NearObject",
         "Fires every frame the player is within num[0] (Radius) units of the "
         "target object."},
        {"OnUsed",
         "Fires when the player presses the USE button while looking at the "
         "target up close. The target object must have its 'usable' flag set."},
        {"EverySeconds", "Fires every num[0] seconds."},
        {"OnAnimFinished",
         "Fires when the target's animation clip reaches its last frame "
         "(animated .glb model objects only)."},
        {"Delay",
         "Exec input arms a timer; the 'after' exec output fires once num[0] "
         "seconds elapse. Re-arming while counting restarts the timer."},
        {"Self",
         "Pure data node exposing the graph's owner object and its live "
         "position."},
        {"ShowObject", "Makes the target visible."},
        {"HideObject", "Makes the target invisible."},
        {"ToggleObject", "Toggles the target's visibility."},
        {"MoveObjectBy", "Instantly shifts the target by (dX, dY, dZ)."},
        {"MoveObjectTo",
        ...
        {"SetPosition",
         ...
         "the params)."},
        {"SpawnObject",
         "Clones the target object into a runtime slot at the linked position "
         "(or the template's own), yaw = num[0] degrees. The object OUTPUT is "
         "the clone, not the template - wire it into further actions. Pool of "
         "32 live clones."},
        {"DespawnObject",
         "Removes a spawned clone (frees its slot); deactivates an authored "
         "object."},
        {"PlayAnimation",
         "Plays clip named str on the target (animated .glb objects; str \"\" "
         "= the model's first clip). num[0] Loop (1 = loop), num[1] Speed "
         "multiplier (0 = default), num[2] crossfade seconds. NOTE: str holds "
         "the CLIP name; the target comes from an object link or defaults to "
         "self."},
        {"StopAnimation", "Freezes the target's animation on its current pose."},
        {"TeleportPlayer",
         "Teleports the player to the target object's position (or a linked "
         "position) - e.g. respawn points."},
        {"Raycast",
         "On exec, casts a ray from the player's eye along the view direction "
         "up to num[0] (Max Dist) and latches the results: position output = "
         "hit point, object output = hit object (may be none). The 'after' "
         "exec fires right after the cast."},
        {"SetFlashlight",
         "num[0] = 1 turns the player's flashlight master switch on, 0 off."},
        {"SetStickCurve",
         "Changes the analog stick response curve live. num[0] Stick: 0 left, "
         "1 right, 2 both. num[1] Curve: 0 Linear, 1 Exponential, 2 S-Curve. "
         "num[2] Exponent >= 1."},
        {"SetSky", "Sets the sky color; num[0..2] = RGB, each 0..1."},
        {"SwitchScene",
         "Loads the scene named str (applied after this frame's scripts)."},
        {"LoadLayer",
         "Starts streaming the layer's assets into memory; activates its "
         "objects when resident."},
        {"UnloadLayer",
         "Deactivates the layer's objects and frees assets no other loaded "
         "layer uses."},
        {"IsLayerLoaded", "Pure bool: is the layer fully loaded?"},
        {"SetGrading",
         "Applies the color grading preset named str; \"\" restores the "
         "ungraded image. Persists across scene changes."},
        {"SetFog", "num[0] = 1 re-applies the scene's fog, 0 disables it."},
        {"SetBloom", "Bloom amount num[0] 0..1 (0 = off)."},
        {"SetGrain", "Film grain amount num[0] 0..1 (0 = off)."},
        {"SetDof",
         "Depth of field. num[3] Mode: 0 = set custom num[0] Focus / num[1] "
         "Range / num[2] Amount 0..1, 1 = off, 2 = restore the scene's "
         "authored setting. A linked position replaces Focus with the live "
         "player-to-point distance."},
        {"SetParticles", "num[0] = 1/0: global switch for all particle emitters."},
        {"SetDisplayMode",
         "Switches the scan mode. num[0] Mode: 0 interlaced, 1 progressive "
         "480p, 2 1080i. num[1] Confirm seconds > 0 shows a keep-or-revert "
         "prompt with automatic rollback."},
        {"SetWidescreen", "num[0] = 1 re-fits the projection for 16:9, 0 for 4:3."},
        {"SetAmbience",
         "Repaints the sky from the ambience preset named str (lighting/fog "
         "are baked per scene; only the sky changes live)."},
        {"PlaySequence",
         "Starts the cutscene sequence named str; retriggering restarts it."},
        {"StopSequence", "Stops the active cutscene."},
        {"ShowHud", "Shows all HUD images."},
        {"HideHud", "Hides all HUD images."},
        {"ToggleHud", "Toggles all HUD images."},
        {"ShowText",
         "Shows the on-screen text named str. num[0] Seconds > 0 auto-hides "
         "after that long; 0 = stays until Hide Text."},
        {"HideText", "Hides the on-screen text named str."},
        {"PlayMusic",
         "Plays the music track str (use the exact track path from the "
         "project context). num[0] Volume 0..100, num[1] Loop (1 = loop)."},
        {"StopMusic", "Stops the music."},
        {"SetMusicVolume", "num[0] Volume 0..100."},
        {"PlaySound",
         "Plays the sound effect str (exact path from the project context). "
         "num[0] Volume 0..100, num[1] Channel 0..23 or -1 = auto-rotate."},
        {"SetValue", "Sets the save value named str to num[0]."},
        {"AddValue", "Adds num[0] to the save value named str."},
        {"ValueAtLeast",
         "Pure bool: save value named str >= num[0], evaluated fresh every "
         "frame."},
        {"GetSaveValue", "Pure text output of the save value named str."},
        {"SetSaveText",
         "Sets the save text named str to str2 (a linked text overrides "
         "str2)."},
        {"GetSaveText", "Pure text output of the save text named str."},
        {"OpenSaveMenu", "Opens the in-game 3-slot save/load menu."},
        {"SetVarInt", "Sets the global int variable named str to num[0]."},
        {"SetVarBool", "Sets the global bool variable named str to num[0] != 0."},
        {"SetVarPos",
         "Sets the global position variable named str to X/Y/Z (a linked "
         "position overrides the params)."},
        {"GetVarBool", "Pure bool: the global bool variable named str."},
        {"GetVarPos", "Pure position: the global position variable named str."},
        {"VarAtLeast", "Pure bool: global int variable named str >= num[0]."},
        {"GetVarIntText", "Pure text of the global int variable named str."},
        {"OpenMenu", "Opens the menu named str."},
        {"OnMenuEvent",
         "Fires the frame a menu entry with Flow event name str is selected. "
         "Also usable as a bool source."},
        {"PosToText", "Pure converter: linked position -> text."},
        {"BoolToText", "Pure converter: linked bool -> text."},
        {"Log",
         "Prints str followed by every wired text input to the game log "
         "(debug builds)."},
        {"And", "Pure bool gate: AND over all wired bool inputs."},
        {"Nand", "Pure bool gate: NOT AND over all wired bool inputs."},
        {"Or", "Pure bool gate: OR over all wired bool inputs."},
        {"Not", "Pure bool gate: NOT of the folded bool inputs."},
        {"Xor", "Pure bool gate: XOR over all wired bool inputs."},
        {"Xnor", "Pure bool gate: NOT XOR over all wired bool inputs."},
        {"OnCondition",
         "Trigger that fires on the RISING EDGE of its bool input - the "
         "bridge from logic gates back to exec flow."},
    };
...
static std::string nodeCatalogLine(const FlowNodeType& t) {
    ...
    if (const char* d = nodeDoc(t.key)) o << " " << d;
    return o.str();
}
