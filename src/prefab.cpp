...
BakeReport bakeToModel(const Project& p, const Prefab& pf) {
    ...
    // Re-baking over a PREVIOUS bake is the normal update path, but the stem
    // comes from the prefab's NAME - and "box" is exactly what a hand-made
    // model is also called. Refuse to clobber anything the bake did not write.
    auto bakeOwned = [](const fs::path& path) {
        std::ifstream f(path);
        if (!f) return false;
        std::string line;
        std::getline(f, line);
    ...
    for (const char* ext : {".obj", ".mtl"}) {
        ...
        if (fs::exists(out, ec) && !bakeOwned(out)) {
