...
// second one is why the first fix read as a partial fix.
//
// THE TOOL. The binding moves off the scene object into its own window (Tools >
// Foot IK, docs/foot-ik.md) and its own project section: one rig per animated
// MODEL asset - six bone names, the distances, per-clip rules - and an object
// keeps the instance switch alone. That is the shape the request asked for ("per
// model/animacja, ale mozna wlaczyc/wylaczyc na graczu") and the shape an NPC
// needs: a rig is authored once for a character and every instance that wants it
// says so. autoDetect covers Mixamo/Rigify/Unreal/plain naming, validate()
// reports an ambiguous bone rather than picking one, and "Measure from model"
// reads the sole offset out of the bind pose - 0.103 measured against 0.100
// authored on the example character, which is also how the reported problem was
// ruled OUT as a scale or offset error before the real cause was found.
//
// THE DESCENT. A descending character needs more downward reach than a level one
// and got the same. The grade (level/up/down) is now decided from three signals -
// filtered root vertical speed, the walker's own ikGroundDrop, and an ahead
// ground probe - and feeds both the gates and the neural predictor's feature
// vector. Measured on a fixed PCSX2 route, same binary as its control:
// unsupported frames 13 % -> 3 % on the 22 cm flight, 7 % on the 11 cm one with
// the longest streak at 0.07 s.
//
// THE STANDING HANG, which the trace disagreed with me about. I read a
// one-step straddle as the cause and was told plainly the feet were level - and
// they were: both grounds at 0.880, and the free foot's TARGET sat 2 mm above
// its own sole. The toe-clearance path was lifting a foot that was not swinging,
// because "moving" was read off foot speed, which is nonzero in a standing idle.
// It now needs the OBJECT to be moving, or the stillness timer to say the
// character is not standing: hang 10 frames (6-frame streak) -> 2 frames (1).
//
// THE PELVIS, third iteration and the two before it were wrong in opposite
// directions - full correction sank the body (-0.169 flat), a fraction of
// maxPelvis hung the foot. It now supplies ONLY the reach the leg itself cannot
// make, measured from the bind-pose hip->knee->ankle span: flat-ground pelvis
// offset -0.047 with weighted-feet-off-target 29 -> 0.
//
// ALSO: the probe overlay (a debug-profile preference, showFootIkProbes) draws
// what the solver actually raycast and where it stopped - which is what the
// "widzieć kiedy pierdolnął" request wanted and what made the standing bug
// legible; a projected shadow option and spawning the player at the editor
// marker's height (y = 1.100 from frame 1, instead of always on terrain); and
// examples/foot-ik-stairs, a curb + two stair runs + a landing, on the CC0
// Quaternius character the user supplied the licence for.
//
// PLUS A 4x FASTER OPEN, reported as "KUREWSKO dlugo": a 39-clip character was
// baking every clip's morph frames to open the project. glbInfo now asks for
// metadata only and the viewport bakes a clip on first use - 23.5 s -> 5.8 s,
// and the lazy bake was proven pixel-identical to the eager one before it was
// kept.
//
// STILL OWED BY A HUMAN. The projected shadow does not visibly render for the
// avatar; the hypothesis is that the receiver patch is built from terrain
// samples under the caster, so it draws under a platform rather than on it -
// unproven, and the option ships doing nothing visible on a stair. The descent
// percentage above was NOT re-measured after the pelvis change (that route's
...
// (v12 and v13 on this branch, renumbered here. Two features may never share a
// number, and main's upscaler entries have held 12 and 13 since before this
// branch existed - so the claim that arrives second renumbers, which is the same
// rule main's own v8 and v14-v16 notes above applied. Nothing on disk changes
